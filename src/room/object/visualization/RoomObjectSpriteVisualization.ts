...
export class RoomObjectSpriteVisualization
  implements IRoomObjectSpriteVisualization
{
  ...
  public getBoundingRectangle(): Rectangle {
    ...
    while (iterator < totalSprites) {
      ...
      if (sprite && sprite.texture && sprite.visible) {
        const offsetX = sprite.flipH
          ? -sprite.width + sprite.offsetX
          : sprite.offsetX;
        const offsetY = sprite.flipV
          ? -sprite.height + sprite.offsetY
          : sprite.offsetY;
