...
export class RoomSpriteCanvas implements IRoomRenderingCanvas {
  ...
  private renderObject(
    ...
    for (const sprite of visualization.sprites.values()) {
      ...
      if (sprite.flipH) {
        const checkX =
          x + -(texture.width + -sprite.offsetX) + this._screenOffsetX;

        if (
          !this.isSpriteVisible(checkX, spriteY, texture.width, texture.height)
        )
          continue;
      } else if (sprite.flipV) {
        const checkY =
          y + -(texture.height + -sprite.offsetY) + this._screenOffsetY;

        if (
          !this.isSpriteVisible(spriteX, checkY, texture.width, texture.height)
        )
          continue;
      } else {
        if (
          !this.isSpriteVisible(spriteX, spriteY, texture.width, texture.height)
        )
          continue;
      }
