...
export class ObjectDataBase implements IObjectData {
  ...
  public initializeFromRoomObjectModel(model: IRoomObjectModel): void {
    this._uniqueNumber = model.getValue<number>(
      RoomObjectVariable.FURNITURE_UNIQUE_SERIAL_NUMBER,
    );
    this._uniqueSeries = model.getValue<number>(
      RoomObjectVariable.FURNITURE_UNIQUE_EDITION_SIZE,
    );
  }
