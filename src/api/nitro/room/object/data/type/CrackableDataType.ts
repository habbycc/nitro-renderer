...
export class CrackableDataType extends ObjectDataBase implements IObjectData {
  ...
  public initializeFromRoomObjectModel(model: IRoomObjectModel): void {
    ...
    this._state = model.getValue<string>(
      RoomObjectVariable.FURNITURE_CRACKABLE_STATE,
    );
    this._hits = model.getValue<number>(
      RoomObjectVariable.FURNITURE_CRACKABLE_HITS,
    );
    this._target = model.getValue<number>(
      RoomObjectVariable.FURNITURE_CRACKABLE_TARGET,
    );
  }
