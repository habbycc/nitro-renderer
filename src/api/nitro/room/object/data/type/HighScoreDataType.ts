...
export class HighScoreDataType extends ObjectDataBase implements IObjectData {
  ...
  public initializeFromRoomObjectModel(model: IRoomObjectModel): void {
    this._scoreType = model.getValue<number>(
      RoomObjectVariable.FURNITURE_HIGHSCORE_SCORE_TYPE,
    );
    this._clearType = model.getValue<number>(
      RoomObjectVariable.FURNITURE_HIGHSCORE_CLEAR_TYPE,
    );
    this._entries = [];

    const totalEntries = model.getValue<number>(
      RoomObjectVariable.FURNITURE_HIGHSCORE_DATA_ENTRY_COUNT,
    );
    ...
    while (i < totalEntries) {
      ...
      data.score = model.getValue<number>(
        RoomObjectVariable.FURNITURE_HIGHSCORE_DATA_ENTRY_BASE_SCORE_ + i,
      );
      data.users = model.getValue<string[]>(
        RoomObjectVariable.FURNITURE_HIGHSCORE_DATA_ENTRY_BASE_USERS_ + i,
      );
