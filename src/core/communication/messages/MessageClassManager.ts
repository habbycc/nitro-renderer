...
export class MessageClassManager {
  ...
  public registerMessages(configuration: IMessageConfiguration): void {
    for (const [header, handler] of configuration.events)
      this.registerMessageEventClass(header, handler);

    for (const [header, handler] of configuration.composers)
  ...
  public getEventId(event: IMessageEvent): number {
    ...
    //@ts-ignore
    const name = (
      event instanceof MessageEvent ? event.constructor : event
    ) as Function;
