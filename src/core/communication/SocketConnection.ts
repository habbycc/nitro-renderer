...
export class SocketConnection extends EventDispatcher implements IConnection {
  ...
  constructor(
    ...
    stateListener: IConnectionStateListener,
  ) {
    super();
  ...
  private createSocket(socketUrl: string): void {
    ...
    this._socket.addEventListener(
      WebSocketEventEnum.CONNECTION_OPENED,
      this.onOpen,
    );
    this._socket.addEventListener(
      WebSocketEventEnum.CONNECTION_CLOSED,
      this.onClose,
    );
    this._socket.addEventListener(
      WebSocketEventEnum.CONNECTION_ERROR,
      this.onError,
    );
    this._socket.addEventListener(
      WebSocketEventEnum.CONNECTION_MESSAGE,
      this.onMessage,
    );
  }
  ...
  public send(...composers: IMessageComposer<unknown[]>[]): boolean {
    ...
    for (const composer of composers) {
      ...
      if (header === -1) {
        if (NitroConfiguration.getValue<boolean>('system.packet.log'))
      ...
      if (!encoded) {
        if (NitroConfiguration.getValue<boolean>('system.packet.log'))
          NitroLogger.log('Encoding Failed', composer.constructor.name);
