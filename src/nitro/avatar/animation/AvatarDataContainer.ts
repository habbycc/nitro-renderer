...
export class AvatarDataContainer implements IAvatarDataContainer {
  ...
  constructor(k: IAssetAnimationAvatar) {
    ...
    this._colorTransform = new AdjustmentFilter({
      red: this._r / 255,
      green: this._g / 255,
      blue: this._b / 255,
      alpha: this._alphaMultiplier,
    });
    this._colorMap = this.generatePaletteMapForGrayscale(
      this._backGround,
      this._foreGround,
    );
  }
  ...
  private generatePaletteMapForGrayscale(
    ...
    for (let i = 0; i < 256; i++) {
      if (
        _local_21 == redBackground &&
        _local_22 == greenBackground &&
        _local_23 == blueBackground
      ) {
        _local_20 = 0;
      }
      ...
      _local_16.push(
        (_local_20 << 24) | (_local_21 << 16) | (_local_22 << 8) | _local_23,
      );
      _local_17.push(
        (_local_20 << 24) | (_local_21 << 16) | (_local_22 << 8) | _local_23,
      );
