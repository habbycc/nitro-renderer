...
export class Animation implements IAnimation {
  ...
  constructor(k: AvatarStructure, _arg_2: IAssetAnimation) {
    ...
    if (_arg_2.sprites && _arg_2.sprites.length) {
      ...
      for (const sprite of _arg_2.sprites)
        this._spriteData.push(new SpriteDataContainer(this, sprite));
    }

    if (_arg_2.avatars && _arg_2.avatars.length)
      this._avatarData = new AvatarDataContainer(_arg_2.avatars[0]);

    if (_arg_2.directions && _arg_2.directions.length)
      this._directionData = new DirectionDataContainer(_arg_2.directions[0]);
    ...
    if (_arg_2.adds && _arg_2.adds.length) {
      ...
      for (const add of _arg_2.adds)
        this._addData.push(new AddDataContainer(add));
    }
  ...
  private parseFrames(
    frames: AvatarAnimationLayerData[][],
    _arg_2: IAssetAnimationFrame[],
    _arg_3: AvatarStructure,
  ): void {
      ...
      while (index < repeats) {
        ...
        if (frame.bodyparts && frame.bodyparts.length) {
          for (const bodyPart of frame.bodyparts) {
            ...
            const layer = new AvatarAnimationLayerData(
              bodyPart,
              AvatarAnimationLayerData.BODYPART,
              definition,
            );
