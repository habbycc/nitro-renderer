...
export class AvatarImageCache {
  ...
  public setGeometryType(k: string): void {
    ...
    if (
      (this._geometryType === GeometryType.SITTING &&
        k === GeometryType.VERTICAL) ||
      (this._geometryType === GeometryType.VERTICAL &&
        k === GeometryType.SITTING) ||
      (this._geometryType === GeometryType.SNOWWARS_HORIZONTAL &&
        (k = GeometryType.SNOWWARS_HORIZONTAL))
    ) {
  ...
  public getImageContainer(
    ...
    if (_local_7.definition.startFromFrameZero)
      frameCount -= _local_7.startFrame;
    ...
    if (!(!_local_7 || !_local_7.definition)) {
      if (_local_7.definition.isAnimation) {
        ...
        const _local_16 = this._structure.getAnimation(
          _local_7.definition.state + '.' + _local_7.actionParameter,
        );
        const _local_17 = frameNumber - _local_7.startFrame;

        if (_local_16) {
          const _local_18 = _local_16.getLayerData(
            _local_17,
            k,
            _local_7.overridingAction,
          );
          ...
          if (_local_18) {
            ...
            if (_local_18.type === AvatarAnimationLayerData.BODYPART) {
              ...
              _local_5 = _local_15;
            } else if (_local_18.type === AvatarAnimationLayerData.FX)
              _local_5 = _local_15;
    ...
    if (!_local_13 || _arg_3) {
      const _local_19 = this._structure.getParts(
        k,
    ...
    if (!_local_14 || _arg_3) {
      ...
      if (_local_14 && !_arg_3) {
        if (_local_14.isCacheable)
          _local_13.updateImageContainer(_local_14, frameCount);
      } else {
        return null;
      }
    }
  ...
  private renderBodyPart(
    ...
    const isFlipped =
      AvatarDirectionAngle.DIRECTION_IS_FLIPPED[direction] || false;
    ...
    while (containerIndex >= 0) {
      ...
      if (
        !(
          direction == 7 &&
          (container.partType === 'fc' || container.partType === 'ey')
        )
      ) {
        if (!(container.partType === 'ri' && !container.partId)) {
          ...
          if (isFlipped) {
            if (
              ...
              if (container.flippedPartType !== partType)
                partType = container.flippedPartType;
            }
          }
          ...
          if (asset) {
            ...
            if (!texture || !texture.valid || !texture.baseTexture) {
              ...
              this._unionImages.push(
                new ImageData(texture, asset.rectangle, offset, flipH, color),
              );
            }
          }
        }
    ...
    const offset = new Point(
      -imageData.regPoint.x,
      canvasOffset - imageData.regPoint.y,
    );
    ...
    return new AvatarImageBodyPartContainer(
      imageData.container,
      offset,
      isCacheable,
    );
