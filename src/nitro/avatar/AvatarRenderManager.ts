...
export class AvatarRenderManager
  ...
  implements IAvatarRenderManager
{
  ...
  constructor() {
    ...
    this.onAvatarAssetDownloaderReady =
      this.onAvatarAssetDownloaderReady.bind(this);
    this.onAvatarAssetDownloaded = this.onAvatarAssetDownloaded.bind(this);
    this.onEffectAssetDownloaderReady =
      this.onEffectAssetDownloaderReady.bind(this);
    this.onEffectAssetDownloaded = this.onEffectAssetDownloaded.bind(this);
    this.onAvatarStructureDownloadDone =
      this.onAvatarStructureDownloadDone.bind(this);
  }
  ...
  public onInit(): void {
    ...
    if (!this._avatarAssetDownloadManager) {
      ...
      this._avatarAssetDownloadManager.addEventListener(
        AvatarAssetDownloadManager.DOWNLOADER_READY,
        this.onAvatarAssetDownloaderReady,
      );
      this._avatarAssetDownloadManager.addEventListener(
        AvatarAssetDownloadManager.LIBRARY_LOADED,
        this.onAvatarAssetDownloaded,
      );
    }
    ...
    if (!this._effectAssetDownloadManager) {
      ...
      this._effectAssetDownloadManager.addEventListener(
        EffectAssetDownloadManager.DOWNLOADER_READY,
        this.onEffectAssetDownloaderReady,
      );
      this._effectAssetDownloadManager.addEventListener(
        EffectAssetDownloadManager.LIBRARY_LOADED,
        this.onEffectAssetDownloaded,
      );
    }
  ...
  public onDispose(): void {
    if (this._avatarAssetDownloadManager) {
      this._avatarAssetDownloadManager.removeEventListener(
        AvatarAssetDownloadManager.DOWNLOADER_READY,
        this.onAvatarAssetDownloaderReady,
      );
    ...
    if (this._effectAssetDownloadManager) {
      this._effectAssetDownloadManager.removeEventListener(
        EffectAssetDownloadManager.DOWNLOADER_READY,
        this.onEffectAssetDownloaderReady,
      );
  ...
  private loadActions(): void {
    const defaultActions = NitroConfiguration.getValue<string>(
      'avatar.default.actions',
    );

    if (defaultActions)
      this._structure.initActions(GetAssetManager(), defaultActions);
    ...
    try {
      request.open(
        'GET',
        NitroConfiguration.getValue<string>('avatar.actions.url'),
      );
  ...
  private loadFigureData(): void {
    const defaultFigureData = NitroConfiguration.getValue<IFigureData>(
    ...
    structureDownloader.addEventListener(
      AvatarStructureDownload.AVATAR_STRUCTURE_DONE,
      this.onAvatarStructureDownloadDone,
    );
  }
  ...
  private checkReady(): void {
    ...
    if (this.events)
      this.events.dispatchEvent(
        new NitroEvent(AvatarRenderEvent.AVATAR_RENDER_READY),
      );
  }
  ...
  public isFigureContainerReady(container: IAvatarFigureContainer): boolean {
    ...
    return this._avatarAssetDownloadManager.isAvatarFigureContainerReady(
      container,
    );
  }
  ...
  public createAvatarImage(
    ...
    if (
      this._avatarAssetDownloadManager.isAvatarFigureContainerReady(
        figureContainer,
      )
    ) {
    ...
    if (!this._placeHolderFigure)
      this._placeHolderFigure = new AvatarFigureContainer(
        AvatarRenderManager.DEFAULT_FIGURE,
      );

    this._avatarAssetDownloadManager.downloadAvatarFigure(
      figureContainer,
      listener,
    );
    ...
    if (typeIds) {
      ...
      for (const id of typeIds) {
        if (!container.hasPartType(id)) {
          ...
          if (setType) {
            const figurePartSet = setType.getPartSet(
              container.getPartSetId(id),
            );
  ...
  public getFigureClubLevel(
    container: IAvatarFigureContainer,
    gender: string,
    searchParts: string[] = null,
  ): number {
    ...
    if (!searchParts)
      searchParts = this._structure.getBodyPartsUnordered(AvatarSetType.FULL);
    ...
    for (const part of searchParts) {
      ...
      if (parts.indexOf(part) === -1)
        clubLevel = Math.max(set.optionalFromClubLevel(gender), clubLevel);
    }
  ...
  public isValidFigureSetForGender(setId: number, gender: string): boolean {
    ...
    return !!(
      partSet &&
      (partSet.gender.toUpperCase() === 'U' ||
        partSet.gender.toUpperCase() === gender.toUpperCase())
    );
  }

  public getFigureStringWithFigureIds(
    figure: string,
    gender: string,
    _arg_3: number[],
  ): string {
    ...
    for (const partSet of partSets) {
      container.savePartData(
        partSet.type,
        partSet.id,
        container.getColourIds(partSet.type),
      );
    }
