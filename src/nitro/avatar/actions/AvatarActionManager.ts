...
export class AvatarActionManager {
  ...
  private filterActions(actions: IActiveActionData[]): IActiveActionData[] {
    ...
    for (const action of actions) {
      ...
      if (localAction)
        preventions = preventions.concat(
          localAction.getPrevents(action.actionParameter),
        );
    }
    ...
    for (const action of actions) {
      ...
      if (action.actionType === 'fx')
        actionType = actionType + ('.' + action.actionParameter);
  ...
  private sortByPrecedence(
    actionOne: IActiveActionData,
    actionTwo: IActiveActionData,
  ): number {
