...
export class AvatarAssetDownloadManager extends EventDispatcher {
  ...
  private loadFigureMap(): void {
    ...
    try {
      request.open(
        'GET',
        NitroConfiguration.getValue<string>('avatar.figuremap.url'),
      );
      ...
      request.onloadend = (e) => {
        if (request.responseText) {
          ...
          this.dispatchEvent(
            new NitroEvent(AvatarAssetDownloadManager.DOWNLOADER_READY),
          );
        }
      };
  ...
  private processFigureMap(data: any): void {
    ...
    for (const library of data) {
      ...
      const downloadLibrary = new AvatarAssetDownloadLibrary(
        id,
        revision,
        this._assets,
      ...
      downloadLibrary.addEventListener(
        AvatarRenderLibraryEvent.DOWNLOAD_COMPLETE,
  ...
  private onLibraryLoaded(event: AvatarRenderLibraryEvent): void {
    ...
    for (const [figure, libraries] of this._incompleteFigures.entries()) {
      ...
      if (isReady) {
        ...
        this.dispatchEvent(
          new NitroEvent(AvatarAssetDownloadManager.LIBRARY_LOADED),
        );
      }
    }
  ...
  public isAvatarFigureContainerReady(
