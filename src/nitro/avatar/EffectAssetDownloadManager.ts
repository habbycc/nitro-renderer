...
import {
  AvatarRenderEffectLibraryEvent,
  AvatarRenderEvent,
  NitroEvent,
} from '../../events';
...
export class EffectAssetDownloadManager extends EventDispatcher {
  ...
  constructor(assets: IAssetManager, structure: AvatarStructure) {
    ...
    this._missingMandatoryLibs = NitroConfiguration.getValue<string[]>(
    ...
    this._structure.renderManager.events.addEventListener(
  ...
  private loadEffectMap(): void {
    ...
    try {
      request.open(
        'GET',
        NitroConfiguration.getValue<string>('avatar.effectmap.url'),
      );
      ...
      request.onloadend = (e) => {
        if (request.responseText) {
          ...
          this.dispatchEvent(
            new NitroEvent(EffectAssetDownloadManager.DOWNLOADER_READY),
          );
        }
      };
  ...
  private processEffectMap(data: any): void {
    ...
    for (const effect of data) {
      ...
      const downloadLibrary = new EffectAssetDownloadLibrary(
        lib,
        revision,
        this._assets,
        NitroConfiguration.getValue<string>('avatar.asset.effect.url'),
      );

      downloadLibrary.addEventListener(
        AvatarRenderEffectLibraryEvent.DOWNLOAD_COMPLETE,
  ...
  public downloadAvatarEffect(
    id: number,
    listener: IAvatarEffectListener,
  ): void {
  ...
  private onLibraryLoaded(event: AvatarRenderEffectLibraryEvent): void {
    ...
    for (const [id, libraries] of this._incompleteEffects.entries()) {
      ...
      if (isReady) {
        ...
        this.dispatchEvent(
          new NitroEvent(EffectAssetDownloadManager.LIBRARY_LOADED),
        );
      }
    }
  ...
  private downloadLibrary(library: EffectAssetDownloadLibrary): void {
    ...
    if (
      this._pendingDownloadQueue.indexOf(library) >= 0 ||
      this._currentDownloads.indexOf(library) >= 0
    )
      return;
