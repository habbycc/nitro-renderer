...
export class AvatarImage implements IAvatarImage, IAvatarEffectListener {
  ...
  constructor(
    ...
    if (_arg_3 == null) {
      _arg_3 = new AvatarFigureContainer(
        'hr-893-45.hd-180-2.ch-210-66.lg-270-82.sh-300-91.wa-2007-.ri-1-',
      );
    }
    this._figure = _arg_3;
    this._cache = new AvatarImageCache(
      this._structure,
      this,
      this._assets,
      this._scale,
    ...
    this.setDirection(
      ...
      AvatarImage.DEFAULT_DIRECTION,
    );
    this._actions = [];
    this._defaultAction = new ActiveActionData(AvatarAction.POSTURE_STAND);
    this._defaultAction.definition = this._structure.getActionDefinition(
      AvatarImage.DEFAULT_ACTION,
    );
    this.resetActions();
  ...
  private getFullImageCacheKey(): string {
    ...
    if (
      this._sortedActions.length == 1 &&
      this._mainDirection == this._headDirection
    ) {
      return (
        this._mainDirection +
    ...
    if (this._sortedActions.length == 2) {
      for (const k of this._sortedActions) {
        if (
          k.actionType == 'fx' &&
          (k.actionParameter == '33' ||
            k.actionParameter == '34' ||
            k.actionParameter == '35' ||
            k.actionParameter == '36')
        ) {
        ...
        if (
          k.actionType == 'fx' &&
          (k.actionParameter == '38' || k.actionParameter == '39')
        ) {
          return (
            this._mainDirection +
            '_' +
  ...
  private getBodyParts(k: string, _arg_2: string, _arg_3: number): string[] {
    if (
      !(_arg_3 == this._cachedBodyPartsDirection) ||
      !(_arg_2 == this._cachedBodyPartsGeometryType) ||
      !(k == this._cachedBodyPartsAvatarSet)
    ) {
  ...
  public getAvatarPartsForCamera(k: string): void {
    ...
    const _local_2 = this._structure.getCanvas(
      this._scale,
      this._mainAction.definition.geometryType,
    );
    if (_local_2 == null) {
      return;
    }
    const _local_3 = this.getBodyParts(
      k,
      this._mainAction.definition.geometryType,
      this._mainDirection,
    );
    ...
    while (_local_6 >= 0) {
      ...
      const _local_5 = this._cache.getImageContainer(
        _local_4,
        this._frameCounter,
        true,
      );
  ...
  public getImage(
    ...
    const avatarCanvas = this._structure.getCanvas(
      this._scale,
      this._mainAction.definition.geometryType,
    );
    ...
    if (
      this._image &&
      (this._image.width !== avatarCanvas.width ||
        this._image.height !== avatarCanvas.height)
    ) {
    ...
    const _local_6 = this.getBodyParts(
      setType,
      this._mainAction.definition.geometryType,
      this._mainDirection,
    );
    ...
    if (this._avatarSpriteData) {
      ...
      if (this._avatarSpriteData.colorTransform)
        container.filters.push(this._avatarSpriteData.colorTransform);
      ...
      if (this._avatarSpriteData.paletteIsGrayscale) {
        ...
        container.filters.push(
          new PaletteMapFilter(
            this._avatarSpriteData.reds,
            PaletteMapFilter.CHANNEL_RED,
          ),
        );
      }
    }
    ...
    if (!cache) {
      return TextureUtils.generateTexture(
        container,
        new Rectangle(0, 0, avatarCanvas.width, avatarCanvas.height),
      );
    }
  ...
  public getImageAsSprite(setType: string, scale: number = 1): Sprite {
    ...
    const setTypes = this.getBodyParts(
      setType,
      this._mainAction.definition.geometryType,
      this._mainDirection,
    );
  ...
  public endActionAppends(): void {
    ...
    for (const k of this._sortedActions) {
      if (k.actionType === AvatarAction.EFFECT) {
        if (
          !this._effectManager.isAvatarEffectReady(parseInt(k.actionParameter))
        )
          this._effectManager.downloadAvatarEffect(
      ...
      case AvatarAction.POSTURE:
        switch (_local_3) {
          ...
          case AvatarAction.SNOWWAR_THROW:
            if (
              _local_3 === AvatarAction.POSTURE_LAY ||
              _local_3 === AvatarAction.POSTURE_LAY ||
              _local_3 === AvatarAction.POSTURE_LAY
            ) {
  ...
  private isHeadTurnPreventedByAction(): boolean {
    ...
    for (const _local_3 of this._sortedActions) {
      _local_2 = this._structure.getActionDefinitionWithState(
        _local_3.actionType,
      );
      if (
        !(_local_2 == null) &&
        _local_2.getPreventHeadTurn(_local_3.actionParameter)
      ) {
        k = true;
      }
    }
  ...
  private sortActions(): boolean {
    ...
    if (!this._sortedActions) {
      ...
      this._canvasOffsets = this._structure.getCanvasOffsets(
        this._sortedActions,
        this._scale,
        this._mainDirection,
      );
      ...
      for (const _local_4 of this._sortedActions) {
        this._currentActionsString =
          this._currentActionsString +
          (_local_4.actionType + _local_4.actionParameter);
  ...
  private setActionsToParts(): void {
    ...
    for (const k of this._sortedActions) {
      if (k && k.definition && k.definition.isAnimation) {
        const _local_2 = this._structure.getAnimation(
