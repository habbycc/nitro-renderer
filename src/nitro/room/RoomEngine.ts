    ...
    IDisposable
{
    ...
    if (roomMap) {
      ...
      if (dimensions) {
        ...
        if (roomObject && roomObject.model)
          roomObject.model.setValue(RoomObjectVariable.ROOM_RANDOM_SEED, seed);
      }
    }
  ...
  public isRoomIdPlayingGame(roomId: number): boolean {
    ...
    return (
      roomInstance.model.getValue<number>(RoomVariableEnum.IS_PLAYING_GAME) > 0
    );
  }
  ...
  private updateRoomCamera(
    ...
    if (
      ...
      const _local_16 =
        roomInstance.model.getValue<number>(RoomVariableEnum.ROOM_MIN_X) - 0.5;
      const _local_17 =
        roomInstance.model.getValue<number>(RoomVariableEnum.ROOM_MIN_Y) - 0.5;
      const _local_18 =
        roomInstance.model.getValue<number>(RoomVariableEnum.ROOM_MAX_X) + 0.5;
      const _local_19 =
        roomInstance.model.getValue<number>(RoomVariableEnum.ROOM_MAX_Y) + 0.5;
      ...
      if (bounds) {
        ...
        if (bounds.width > 1 && bounds.height > 1) {
          _local_29 =
            (bounds.left - _local_33.x - roomGeometry.scale * 0.25) / _local_24;
          _local_31 =
            (bounds.right - _local_33.x + roomGeometry.scale * 0.25) /
            _local_24;
          _local_30 =
            (bounds.top - _local_33.y - roomGeometry.scale * 0.5) / _local_25;
          _local_32 =
            (bounds.bottom - _local_33.y + roomGeometry.scale * 0.5) /
            _local_25;
        } else {
      ...
      if (_local_40 + _local_41 > 0) {
        ...
        canvasRectangle.y +=
          -canvasRectangle.height * (_local_41 / (_local_40 + _local_41));
      } else {
        canvasRectangle.x += -canvasRectangle.width / 2;
  ...
  public setFurnitureStackingHeightMap(
    roomId: number,
    heightMap: IFurnitureStackingHeightMap,
  ): void {
    const instanceData = this.getRoomInstanceData(roomId);
  ...
  public removeRoomObjectFloor(
    ...
    if (_arg_4)
      this.refreshTileObjectMap(roomId, 'RoomEngine.disposeObjectFurniture()');
  }
  ...
  public updateRoomObjectUserFigure(
    ...
    object.processUpdateMessage(
      new ObjectAvatarFigureUpdateMessage(figure, gender, subType, isRiding),
    );
  ...
  private onBadgeImageReadyEvent(k: BadgeImageReadyEvent): void {
    ...
    const listeners =
      this._badgeListenerObjects && this._badgeListenerObjects.get(k.badgeId);
  ...
  public dispatchMouseEvent(
    ...
    if (
      !this.handleRoomDragging(canvas, x, y, type, altKey, ctrlKey, shiftKey)
    ) {
      if (
            ...
            if (camera) {
              if (this.useOffsetScrolling) {
                ...
                camera.resetLocation(
                  new Vector3d(-canvas.screenOffsetX, -canvas.screenOffsetY),
                );
              }
  ...
  private setMouseButton(
    ...
    if (
      ...
      if (instanceData) {
        if (instanceData.addButtonMouseCursorOwner(category + '_' + objectId))
  ...
  private setMouseDefault(
    ...
    if (instanceData) {
      if (instanceData.removeButtonMouseCursorOwner(category + '_' + objectId))
        this._mouseCursorUpdate = true;
    }
  }
  ...
  public processRoomObjectPlacement(
    ...
    if (
      !roomInstance ||
      roomInstance.model.getValue<number>(RoomVariableEnum.ROOM_IS_PUBLIC) !== 0
    )
      return false;
  ...
  public setObjectMoverIconSprite(
    ...
    if (_arg_3) {
      ...
      if (this._roomContentLoader) {
        if (category === RoomObjectCategory.FLOOR) {
          type =
            this._roomContentLoader.getFurnitureFloorNameForTypeId(objectId);
          colorIndex =
            this._roomContentLoader.getFurnitureFloorColorIndex(objectId);
          ...
          colorIndex =
            this._roomContentLoader.getFurnitureWallColorIndex(objectId);
        }
    ...
    if (_local_15) {
      ...
      _local_15.y =
        this._activeRoomActiveCanvasMouseY - imageResult.data.height / 2;
    }
  }
        ...
        switch (category) {
          ...
          case RoomObjectCategory.WALL: {
            color = roomObject.model
              .getValue<number>(RoomObjectVariable.FURNITURE_COLOR)
              .toString();
    ...
    switch (objectCategory) {
      ...
      case RoomObjectCategory.UNIT:
        if (
          ...
          if (posture)
            model.setValue(RoomObjectVariable.FIGURE_POSTURE, posture);
        }
        break;
  ...
  private getRenderingCanvasOverlay(k: IRoomRenderingCanvas): NitroSprite {
    ...
    return (
      (displayObject.getChildByName(RoomEngine.OVERLAY) as NitroSprite) || null
    );
  }
  ...
  public refreshTileObjectMap(k: number, _arg_2: string): void {
    ...
    if (tileObjectMap)
      tileObjectMap.populate(this.getRoomObjects(k, RoomObjectCategory.FLOOR));
  }
