...
export class RoomPreviewer {
  ...
  private createRoomForPreview(): void {
    if (this.isRoomEngineReady) {
      ...
      this._roomEngine.createRoomInstance(
        this._previewRoomId,
        planeParser.getMapData(),
      );
    ...
    if (this.isRoomEngineReady) {
      ...
      if (
        this._roomEngine.addFurnitureFloor(
          ...
          classId,
          new Vector3d(
            RoomPreviewer.PREVIEW_OBJECT_LOCATION_X,
            RoomPreviewer.PREVIEW_OBJECT_LOCATION_Y,
            0,
          ),
          direction,
          0,
        ...
        if (roomObject && extra)
          roomObject.model.setValue(RoomObjectVariable.FURNITURE_EXTRAS, extra);
  ...
  public updateUserGesture(gestureId: number): void {
    if (this.isRoomEngineReady)
      this._roomEngine.updateRoomObjectUserGesture(
        this._previewRoomId,
        RoomPreviewer.PREVIEW_OBJECT_ID,
        gestureId,
      );
  ...
  public updateUserEffect(effectId: number): void {
    if (this.isRoomEngineReady)
      this._roomEngine.updateRoomObjectUserEffect(
        this._previewRoomId,
        RoomPreviewer.PREVIEW_OBJECT_ID,
        effectId,
      );
  ...
  public updateObjectUserFigure(
    ...
    if (this.isRoomEngineReady)
      return this._roomEngine.updateRoomObjectUserFigure(
        this._previewRoomId,
        RoomPreviewer.PREVIEW_OBJECT_ID,
        figure,
        gender,
  ...
  public updateObjectUserAction(
    ...
    if (this.isRoomEngineReady)
      this._roomEngine.updateRoomObjectUserAction(
        this._previewRoomId,
        RoomPreviewer.PREVIEW_OBJECT_ID,
        action,
        value,
  ...
  public updateObjectStuffData(stuffData: IObjectData): void {
    if (this.isRoomEngineReady)
      this._roomEngine.updateRoomObjectFloor(
        this._previewRoomId,
        RoomPreviewer.PREVIEW_OBJECT_ID,
        null,
        null,
  ...
  public changeRoomObjectDirection(): void {
    if (this.isRoomEngineReady) {
      ...
      const direction =
        this._roomEngine.objectEventHandler.getValidRoomObjectDirection(
          roomObject,
          true,
        );
      ...
      switch (this._currentPreviewObjectCategory) {
        case RoomObjectCategory.FLOOR: {
          const floorLocation = new Vector3d(
            RoomPreviewer.PREVIEW_OBJECT_LOCATION_X,
            RoomPreviewer.PREVIEW_OBJECT_LOCATION_Y,
          );
    ...
    if (this._automaticStateChange) {
      ...
      if (
        ...
        this._previousAutomaticStateChangeTime +
          RoomPreviewer.AUTOMATIC_STATE_CHANGE_INTERVAL
      ) {
  ...
  public getRoomCanvas(width: number, height: number): DisplayObject {
    if (this.isRoomEngineReady) {
      ...
      this._roomEngine.setRoomInstanceRenderingCanvasMask(
        this._previewRoomId,
        RoomPreviewer.PREVIEW_CANVAS_ID,
        true,
      );

      const geometry = this._roomEngine.getRoomInstanceGeometry(
        this._previewRoomId,
        RoomPreviewer.PREVIEW_CANVAS_ID,
      );

      if (geometry)
        geometry.adjustLocation(
          new Vector3d(
            RoomPreviewer.PREVIEW_OBJECT_LOCATION_X,
            RoomPreviewer.PREVIEW_OBJECT_LOCATION_Y,
            0,
  ...
  public modifyRoomCanvas(width: number, height: number): void {
    if (this.isRoomEngineReady) {
      ...
      this._roomEngine.initializeRoomInstanceRenderingCanvas(
        this._previewRoomId,
        RoomPreviewer.PREVIEW_CANVAS_ID,
        width,
        height,
  ...
  public updatePreviewObjectBoundingRectangle(point: Point): void {
    ...
    if (objectBounds && point) {
      ...
      if (!this._currentPreviewRectangle) {
        ...
        const bounds = this._currentPreviewRectangle
          .clone()
          .enlarge(objectBounds);

        if (
          bounds.width - this._currentPreviewRectangle.width >
            (this._currentPreviewCanvasWidth -
              this._currentPreviewRectangle.width) >>
              1 ||
          bounds.height - this._currentPreviewRectangle.height >
            (this._currentPreviewCanvasHeight -
              this._currentPreviewRectangle.height) >>
              1 ||
    ...
    if (this.isRoomEngineReady) {
      ...
      if (
        ...
        if (RoomPreviewer.ZOOM_ENABLED) {
          if (
            this._roomEngine.getRoomInstanceRenderingCanvasScale(
              this._previewRoomId,
              RoomPreviewer.PREVIEW_CANVAS_ID,
            ) !== 0.5
          ) {
            ...
            this._currentPreviewRectangle.x =
              this._currentPreviewRectangle.x >> 2;
            this._currentPreviewRectangle.y =
              this._currentPreviewRectangle.y >> 2;
            this._currentPreviewRectangle.width =
              this._currentPreviewRectangle.width >> 2;
            this._currentPreviewRectangle.height =
              this._currentPreviewRectangle.height >> 2;
          }
        } else {
  ...
  public updateObjectRoom(
    ...
    if (this.isRoomEngineReady)
      return this._roomEngine.updateRoomInstancePlaneType(
        this._previewRoomId,
        floorType,
        wallType,
        landscapeType,
  ...
  public updateRoomWallsAndFloorVisibility(
    wallsVisible: boolean,
    floorsVisible: boolean = true,
  ): void {
    if (this.isRoomEngineReady)
      this._roomEngine.updateRoomInstancePlaneVisibility(
        this._previewRoomId,
        wallsVisible,
        floorsVisible,
      );
  ...
  private getCanvasOffset(point: Point): Point {
    ...
    let x =
      -(
        this._currentPreviewRectangle.left + this._currentPreviewRectangle.right
      ) >> 1;
    let y =
      -(
        this._currentPreviewRectangle.top + this._currentPreviewRectangle.bottom
      ) >> 1;
    const height =
      (this._currentPreviewCanvasHeight -
        this._currentPreviewRectangle.height) >>
  ...
  public updatePreviewRoomView(k: boolean = false): void {
    ...
    if (this.isRoomEngineReady) {
      let offset = this._roomEngine.getRoomInstanceRenderingCanvasOffset(
        this._previewRoomId,
        RoomPreviewer.PREVIEW_CANVAS_ID,
      );
      ...
      if (offset) {
        ...
        if (this._currentPreviewRectangle) {
          ...
          if (canvasOffset) {
            this._roomEngine.setRoomInstanceRenderingCanvasOffset(
              this._previewRoomId,
              RoomPreviewer.PREVIEW_CANVAS_ID,
              canvasOffset,
            );
          ...
          if (this._currentPreviewScale !== scale)
  ...
  private onRoomObjectAdded(event: RoomEngineObjectEvent): void {
    if (
      event.roomId === this._previewRoomId &&
      event.objectId === RoomPreviewer.PREVIEW_OBJECT_ID &&
      event.category === this._currentPreviewObjectCategory
    ) {
      this._currentPreviewRectangle = null;
      ...
      if (
        ...
        const sizeZ = roomObject.model.getValue<number>(
          RoomObjectVariable.FURNITURE_SIZE_Z,
        );
        const centerZ = roomObject.model.getValue<number>(
          RoomObjectVariable.FURNITURE_CENTER_Z,
        );

        if (sizeZ !== null || centerZ !== null) {
          this._roomEngine.updateRoomObjectWallLocation(
            event.roomId,
            event.objectId,
  ...
  public getRenderingCanvas(): IRoomRenderingCanvas {
    const renderingCanvas = this._roomEngine.getRoomInstanceRenderingCanvas(
      this._previewRoomId,
      RoomPreviewer.PREVIEW_CANVAS_ID,
    );
