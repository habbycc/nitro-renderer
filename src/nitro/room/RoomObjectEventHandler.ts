...
export class RoomObjectEventHandler
  extends Disposable
  implements IRoomCanvasMouseListener, IRoomObjectEventManager
{
  ...
  private handleRoomObjectMouseLeaveEvent(
    ...
    if (category !== RoomObjectCategory.ROOM) {
      if (category === RoomObjectCategory.UNIT) {
        ...
        if (cursor)
          cursor.processUpdateMessage(new ObjectDataUpdateMessage(0, null));
      }
    }
  ...
  private onRoomObjectWidgetRequestEvent(
    ...
    switch (event.type) {
      ...
      case RoomObjectWidgetRequestEvent.ACHIEVEMENT_RESOLUTION_OPEN:
        this._roomEngine.connection.send(
          new GetResolutionAchievementsMessageComposer(event.objectId, 0),
        );
        return;
      case RoomObjectWidgetRequestEvent.ACHIEVEMENT_RESOLUTION_ENGRAVING:
        eventDispatcher.dispatchEvent(
    ...
    if (eventType)
      this._roomEngine.events.dispatchEvent(
        new RoomEngineObjectEvent(
          ...
          roomId,
          event.objectId,
          this._roomEngine.getRoomObjectCategoryForType(event.objectType),
        ),
      );
  }
    ...
    switch (event.type) {
      ...
      case RoomObjectDataRequestEvent.RODRE_URL_PREFIX:
        event.object.model.setValue(
          ...
          NitroConfiguration.getValue('url.prefix'),
        );
        return;
    }
  }
  ...
  private validateFurnitureLocation(
    ...
    _arg_3: IVector3D,
    _arg_4: IVector3D,
    _arg_5: IFurnitureStackingHeightMap,
  ): Vector3d {
        ...
        if (this._objectPlacementSource !== RoomObjectPlacementSource.CATALOG) {
          if (category === RoomObjectCategory.UNIT) {
            if (selectedData.typeId === RoomObjectType.PET) {
              this._roomEngine.connection.send(
                new PetPlaceComposer(objectId, Math.trunc(x), Math.trunc(y)),
              );
            } else if (selectedData.typeId === RoomObjectType.RENTABLE_BOT) {
              this._roomEngine.connection.send(
                new BotPlaceComposer(objectId, Math.trunc(x), Math.trunc(y)),
              );
            }
          } else if (
            ...
            this._roomEngine.connection.send(
              new FurniturePostItPlaceComposer(objectId, wallLocation),
            );
          } else {
            this._roomEngine.connection.send(
    ...
    switch (operation) {
        ...
        if (this._roomEngine.connection) {
          ...
          if (
            ...
            if (roomObject.type === RoomObjectUserType.MONSTER_PLANT) {
              ...
              if (roomSession) {
                const userData =
                  roomSession.userDataManager.getUserDataByIndex(objectId);
      ...
      case RoomObjectOperationType.OBJECT_PICKUP_PET:
        if (this._roomEngine.connection) {
          ...
          if (session) {
            const userData =
              session.userDataManager.getUserDataByIndex(objectId);
        ...
        if (this._roomEngine.connection) {
          if (category === RoomObjectCategory.FLOOR) {
            ...
            if (wallGeometry) {
              ...
              if (location)
                this._roomEngine.connection.send(
                  new FurnitureWallUpdateComposer(objectId, location),
                );
            }
          } else if (category === RoomObjectCategory.UNIT) {
            const angle = roomObject.getDirection().x % 360;
            ...
            const direction = angle / 45;
            const race = parseInt(
              roomObject.model.getValue<string>(RoomObjectVariable.RACE),
            );
            const roomSession =
              this._roomEngine.roomSessionManager.getSession(roomId);
  ...
  public getValidRoomObjectDirection(
    ...
    if (allowedDirections && allowedDirections.length) {
      ...
      if (_arg_2) _local_6 = (_local_6 + 1) % allowedDirections.length;
        ...
        _local_6 =
          (_local_6 - 1 + allowedDirections.length) % allowedDirections.length;
  ...
  private handleUserPlace(
    ...
    x: number,
    y: number,
    wallGeometry: ILegacyWallGeometry,
  ): boolean {
