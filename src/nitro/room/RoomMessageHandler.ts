...
export class RoomMessageHandler extends Disposable {
  ...
  public setConnection(connection: IConnection) {
    ...
    this._connection.addMessageEvent(
      new FurnitureFloorRemoveEvent(
        this.onFurnitureFloorRemoveEvent.bind(this),
      ),
    );
    this._connection.addMessageEvent(
      new FurnitureFloorUpdateEvent(
        this.onFurnitureFloorUpdateEvent.bind(this),
      ),
    );
    this._connection.addMessageEvent(
    ...
    this._connection.addMessageEvent(
      new ItemDataUpdateMessageEvent(
        this.onItemDataUpdateMessageEvent.bind(this),
      ),
    );
    this._connection.addMessageEvent(
      new OneWayDoorStatusMessageEvent(
        this.onOneWayDoorStatusMessageEvent.bind(this),
      ),
    );
    this._connection.addMessageEvent(
    ...
    this._connection.addMessageEvent(
      new GuideSessionStartedMessageEvent(
        this.onGuideSessionStartedMessageEvent.bind(this),
      ),
    );
    this._connection.addMessageEvent(
      new GuideSessionEndedMessageEvent(
        this.onGuideSessionEndedMessageEvent.bind(this),
      ),
    );
    this._connection.addMessageEvent(
      new GuideSessionErrorMessageEvent(
        this.onGuideSessionErrorMessageEvent.bind(this),
      ),
    );
  }
  ...
  private onRoomPaintEvent(event: RoomPaintEvent): void {
    ...
    if (this._roomCreator) {
      this._roomCreator.updateRoomInstancePlaneType(
        this._currentRoomId,
        floorType,
        wallType,
        landscapeType,
  ...
  private onRoomModelEvent(event: FloorHeightMapEvent): void {
    if (
      !(event instanceof FloorHeightMapEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
    ...
    const wallGeometry = this._roomCreator.getLegacyWallGeometry(
      this._currentRoomId,
    );
    ...
    while (heightIterator >= 0) {
      ...
      while (widthIterator >= 0) {
        wallGeometry.setHeight(
          widthIterator,
          heightIterator,
          this._planeParser.getTileHeight(widthIterator, heightIterator),
        );
        widthIterator--;
      }
  ...
  private onRoomHeightMapEvent(event: RoomHeightMapEvent): void {
    if (
      !(event instanceof RoomHeightMapEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
    ...
    this._roomCreator.setFurnitureStackingHeightMap(
      this._currentRoomId,
      heightMap,
    );
  }
  ...
  private onRoomHeightMapUpdateEvent(event: RoomHeightMapUpdateEvent): void {
    if (
      !(event instanceof RoomHeightMapUpdateEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
    ...
    const heightMap = this._roomCreator.getFurnitureStackingHeightMap(
      this._currentRoomId,
    );
    ...
    while (parser.next()) {
      ...
      heightMap.setStackingBlocked(
        parser.x,
        parser.y,
        parser.isStackingBlocked(),
      );
    ...
    this._roomCreator.refreshTileObjectMap(
      this._currentRoomId,
      'RoomMessageHandler.onRoomHeightMapUpdateEvent()',
    );
  }
  ...
  private onRoomThicknessEvent(event: RoomVisualizationSettingsEvent): void {
    ...
    if (this._roomCreator) {
      this._roomCreator.updateRoomInstancePlaneVisibility(
        this._currentRoomId,
        visibleWall,
        visibleFloor,
      );
      this._roomCreator.updateRoomInstancePlaneThickness(
        this._currentRoomId,
        thicknessWall,
        thicknessFloor,
      );
  ...
  private onRoomRollingEvent(event: ObjectsRollingEvent): void {
    if (
      !(event instanceof ObjectsRollingEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
    ...
    if (unitRollData) {
      ...
      const object = this._roomCreator.getRoomObjectUser(
        this._currentRoomId,
        unitRollData.id,
      );
  ...
  private onObjectsDataUpdateEvent(event: ObjectsDataUpdateEvent): void {
    if (
      !(event instanceof ObjectsDataUpdateEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
  ...
  private onFurnitureAliasesEvent(event: FurnitureAliasesEvent): void {
    if (
      !(event instanceof FurnitureAliasesEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
  ...
  private onFurnitureFloorAddEvent(event: FurnitureFloorAddEvent): void {
    if (
      !(event instanceof FurnitureFloorAddEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
  ...
  private onFurnitureFloorEvent(event: FurnitureFloorEvent): void {
    if (
      !(event instanceof FurnitureFloorEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
  ...
  private onFurnitureFloorRemoveEvent(event: FurnitureFloorRemoveEvent): void {
    if (
      !(event instanceof FurnitureFloorRemoveEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
    ...
    if (parser.delay > 0) {
      setTimeout(() => {
        this._roomCreator.removeRoomObjectFloor(
          this._currentRoomId,
          parser.itemId,
          parser.isExpired ? -1 : parser.userId,
          true,
        );
      }, parser.delay);
    } else {
  ...
  private onFurnitureFloorUpdateEvent(event: FurnitureFloorUpdateEvent): void {
    if (
      !(event instanceof FurnitureFloorUpdateEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
    ...
    this._roomCreator.updateRoomObjectFloorHeight(
      this._currentRoomId,
      item.itemId,
      item.stackHeight,
    );
    this._roomCreator.updateRoomObjectFloorExpiration(
      this._currentRoomId,
      item.itemId,
      item.expires,
    );
  ...
  private onFurnitureWallAddEvent(event: FurnitureWallAddEvent): void {
    if (
      !(event instanceof FurnitureWallAddEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
  ...
  private onFurnitureWallEvent(event: FurnitureWallEvent): void {
    if (
      !(event instanceof FurnitureWallEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
  ...
  private onFurnitureWallRemoveEvent(event: FurnitureWallRemoveEvent): void {
    if (
      !(event instanceof FurnitureWallRemoveEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
    ...
    this._roomCreator.removeRoomObjectWall(
      this._currentRoomId,
      parser.itemId,
      parser.userId,
    );
  ...
  private onFurnitureWallUpdateEvent(event: FurnitureWallUpdateEvent): void {
    if (
      !(event instanceof FurnitureWallUpdateEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
    ...
    const location = wallGeometry.getLocation(
      item.width,
      item.height,
      item.localX,
      item.localY,
    ...
    this._roomCreator.updateRoomObjectWall(
      this._currentRoomId,
      item.itemId,
      location,
      direction,
    ...
    this._roomCreator.updateRoomObjectWallExpiration(
      this._currentRoomId,
      item.itemId,
  ...
  private onFurnitureDataEvent(event: FurnitureDataEvent): void {
    if (
      !(event instanceof FurnitureDataEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
  ...
  private onItemDataUpdateMessageEvent(
    ...
    if (
      !(event instanceof ItemDataUpdateMessageEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
    ...
    this._roomCreator.updateRoomObjectWallItemData(
      this._currentRoomId,
      parser.furnitureId,
      parser.data,
    );
  ...
  private onOneWayDoorStatusMessageEvent(
    event: OneWayDoorStatusMessageEvent,
  ): void {
    if (
      !(event instanceof OneWayDoorStatusMessageEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
  ...
  private onDiceValueMessageEvent(event: DiceValueMessageEvent): void {
    if (
      !(event instanceof DiceValueMessageEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
  ...
  private onRoomUnitDanceEvent(event: RoomUnitDanceEvent): void {
    if (
      !(event instanceof RoomUnitDanceEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
    ...
    this._roomCreator.updateRoomObjectUserAction(
      ...
      RoomObjectVariable.FIGURE_DANCE,
      event.getParser().danceId,
    );
  }

  private onRoomUnitEffectEvent(event: RoomUnitEffectEvent): void {
    if (
      !(event instanceof RoomUnitEffectEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;

    this._roomCreator.updateRoomObjectUserEffect(
      this._currentRoomId,
      event.getParser().unitId,
      event.getParser().effectId,
      event.getParser().delay,
    );
  }

  private onRoomUnitEvent(event: RoomUnitEvent): void {
    if (
      !(event instanceof RoomUnitEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
    ...
    for (const user of users) {
      ...
      this._roomCreator.addRoomObjectUser(
        this._currentRoomId,
        user.roomIndex,
        location,
        direction,
      ...
      if (user.webID === this._ownUserId) {
        this._roomCreator.setRoomSessionOwnUser(
          this._currentRoomId,
          user.roomIndex,
        );
        this._roomCreator.updateRoomObjectUserOwn(
          this._currentRoomId,
          user.roomIndex,
        );
      }

      this._roomCreator.updateRoomObjectUserFigure(
        this._currentRoomId,
        user.roomIndex,
        user.figure,
        user.sex,
      ...
      if (
        RoomObjectUserType.getTypeString(user.userType) ===
        RoomObjectUserType.PET
      ) {
        if (
          this._roomCreator.getPetTypeId(user.figure) === PetType.MONSTERPLANT
        ) {
          this._roomCreator.updateRoomObjectUserPosture(
            this._currentRoomId,
            user.roomIndex,
  ...
  private onRoomUnitExpressionEvent(event: RoomUnitExpressionEvent): void {
    if (
      !(event instanceof RoomUnitExpressionEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
  ...
  private onRoomUnitHandItemEvent(event: RoomUnitHandItemEvent): void {
    if (
      !(event instanceof RoomUnitHandItemEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
  ...
  private onRoomUnitIdleEvent(event: RoomUnitIdleEvent): void {
    if (
      !(event instanceof RoomUnitIdleEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
  ...
  private onRoomUnitInfoEvent(event: RoomUnitInfoEvent): void {
    if (
      !(event instanceof RoomUnitInfoEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
    ...
    this._roomCreator.updateRoomObjectUserFigure(
      this._currentRoomId,
      event.getParser().unitId,
      event.getParser().figure,
      event.getParser().gender,
    );
  }

  private onRoomUnitNumberEvent(event: RoomUnitNumberEvent): void {
    if (
      !(event instanceof RoomUnitNumberEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
    ...
    this._roomCreator.updateRoomObjectUserAction(
      this._currentRoomId,
      parser.unitId,
      RoomObjectVariable.FIGURE_NUMBER_VALUE,
      parser.value,
    );
  }
  ...
  private onRoomUnitRemoveEvent(event: RoomUnitRemoveEvent): void {
    if (
      !(event instanceof RoomUnitRemoveEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;

    this._roomCreator.removeRoomObjectUser(
      this._currentRoomId,
      event.getParser().unitId,
    );
  ...
  private onRoomUnitStatusEvent(event: RoomUnitStatusEvent): void {
    if (
      !(event instanceof RoomUnitStatusEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
    ...
    const zScale =
      roomInstance.model.getValue<number>(RoomVariableEnum.ROOM_Z_SCALE) || 1;
    ...
    for (const status of statuses) {
      ...
      if (status.didMove)
        goal = new Vector3d(status.targetX, status.targetY, status.targetZ);
      ...
      this._roomCreator.updateRoomObjectUserFlatControl(
        this._currentRoomId,
        status.id,
        null,
      );
          ...
          switch (action.action) {
            ...
            case 'sign':
              ...
              this._roomCreator.updateRoomObjectUserAction(
                this._currentRoomId,
                status.id,
                RoomObjectVariable.FIGURE_SIGN,
            ...
            case 'gst':
              ...
              this._roomCreator.updateRoomObjectUserPetGesture(
                this._currentRoomId,
                status.id,
                action.value,
              );
  ...
  private onRoomUnitChatEvent(event: RoomUnitChatEvent): void {
    ...
    this._roomCreator.updateRoomObjectUserGesture(
      this._currentRoomId,
      parser.roomIndex,
      parser.gesture,
    );
    this._roomCreator.updateRoomObjectUserAction(
      this._currentRoomId,
      parser.roomIndex,
      RoomObjectVariable.FIGURE_TALK,
      parser.message.length / 10,
    );
  }

  private onRoomUnitTypingEvent(event: RoomUnitTypingEvent): void {
    if (
      !(event instanceof RoomUnitTypingEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
  ...
  private onPetFigureUpdateEvent(event: PetFigureUpdateEvent): void {
    if (
      !(event instanceof PetFigureUpdateEvent) ||
      !event.connection ||
      !this._roomCreator
    )
      return;
  ...
  private onPetExperienceEvent(event: PetExperienceEvent): void {
    ...
    this._roomCreator.updateRoomObjectUserAction(
      this._currentRoomId,
      parser.roomIndex,
      RoomObjectVariable.FIGURE_GAINED_EXPERIENCE,
  ...
  private onYouArePlayingGameEvent(event: YouArePlayingGameEvent): void {
    ...
    this._roomCreator.setRoomEngineGameMode(
      this._currentRoomId,
      parser.isPlaying,
    );
  }
  ...
  private addRoomObjectFurnitureWall(
    ...
    if (!data.isOldFormat) {
      location = wallGeometry.getLocation(
        data.width,
        data.height,
        data.localX,
        data.localY,
  ...
  private onIgnoreResultEvent(event: IgnoreResultEvent): void {
    ...
    const roomSession = this._roomCreator.roomSessionManager.getSession(
      this._currentRoomId,
    );
  ...
  private onGuideSessionStartedMessageEvent(
    event: GuideSessionStartedMessageEvent,
  ): void {
    const parser = event.getParser();
  ...
  private onGuideSessionEndedMessageEvent(
    k: GuideSessionEndedMessageEvent,
  ): void {
    this.removeGuideMarker();
  }

  private onGuideSessionErrorMessageEvent(
    k: GuideSessionErrorMessageEvent,
  ): void {
    this.removeGuideMarker();
  }
  ...
  private updateGuideMarker(): void {
    ...
    this.setUserGuideStatus(
      this._guideId,
      this._requesterId === userId
        ? AvatarGuideStatus.GUIDE
        : AvatarGuideStatus.NONE,
    );
    this.setUserGuideStatus(
      this._requesterId,
      this._guideId === userId
        ? AvatarGuideStatus.REQUESTER
        : AvatarGuideStatus.NONE,
    );
  }
  ...
  private setUserGuideStatus(userId: number, status: number): void {
    ...
    const userData = roomSession.userDataManager.getDataByType(
      userId,
      RoomObjectType.USER,
    );
    ...
    this._roomCreator.updateRoomObjectUserAction(
      this._currentRoomId,
      userData.roomIndex,
      RoomObjectVariable.FIGURE_GUIDE_STATUS,
      status,
    );
  }
