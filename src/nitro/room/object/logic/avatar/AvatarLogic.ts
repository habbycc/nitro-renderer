...
export class AvatarLogic extends MovingObjectLogic {
  ...
  constructor() {
    ...
    this._newEffect = 0;
    this._blinkingStartTimestamp =
      GetTickerTime() + this.randomBlinkStartTimestamp();
    ...
    if (this._selected && this.object) {
      if (this.eventDispatcher)
        this.eventDispatcher.dispatchEvent(
          new RoomObjectMoveEvent(
            RoomObjectMoveEvent.OBJECT_REMOVED,
            this.object,
          ),
        );
    }
  ...
  private updateModel(time: number, model: IRoomObjectModel): void {
    if (this._talkingEndTimestamp > 0) {
      if (time > this._talkingEndTimestamp) {
        ...
        if (
          ...
          !this._talkingPauseStartTimestamp
        ) {
          this._talkingPauseStartTimestamp =
            time + this.randomTalkingPauseStartTimestamp();
          this._talkingPauseEndTimestamp =
            this._talkingPauseStartTimestamp +
            this.randomTalkingPauseEndTimestamp();
        } else {
          if (
            this._talkingPauseStartTimestamp > 0 &&
            time > this._talkingPauseStartTimestamp
          ) {
            model.setValue(RoomObjectVariable.FIGURE_TALK, 0);
            ...
            if (
              this._talkingPauseEndTimestamp > 0 &&
              time > this._talkingPauseEndTimestamp
            ) {
    ...
    if (
      ...
      time > this._blinkingStartTimestamp
    ) {
    ...
    if (
      this._numberValueEndTimestamp > 0 &&
      time > this._numberValueEndTimestamp
    ) {
  ...
  public processUpdateMessage(message: RoomObjectUpdateMessage): void {
    ...
    if (message instanceof ObjectAvatarPlayingGameUpdateMessage) {
      model.setValue(
        RoomObjectVariable.FIGURE_IS_PLAYING_GAME,
        message.isPlayingGame ? 1 : 0,
      );
    ...
    if (message instanceof ObjectAvatarExpressionUpdateMessage) {
      ...
      this._animationEndTimestamp = AvatarAction.getExpressionTimeout(
        model.getValue<number>(RoomObjectVariable.FIGURE_EXPRESSION),
      );
    ...
    if (message instanceof ObjectAvatarSleepUpdateMessage) {
      model.setValue(
        ...
        message.isSleeping ? 1 : 0,
      );

      if (message.isSleeping) this._blinkingStartTimestamp = -1;
      else
        this._blinkingStartTimestamp =
          this.time + this.randomBlinkStartTimestamp();
    ...
    if (message instanceof ObjectAvatarCarryObjectUpdateMessage) {
      ...
      if (message.itemType === 0) {
        ...
        if (message.itemType < AvatarLogic.MAX_HAND_ID) {
          this._carryObjectEndTimestamp = 0;
          this._allowUseCarryObject =
            message.itemType <= AvatarLogic.MAX_HAND_USE_ID;
        } else {
          this._carryObjectEndTimestamp =
            this._carryObjectStartTimestamp + 1500;
          this._allowUseCarryObject = false;
        }
      }
  ...
  private updateAvatarEffect(
    ...
    if (effect === AvatarLogic.EFFECT_TYPE_SPLASH) {
      this._effectChangeTimeStamp =
        GetTickerTime() + AvatarLogic.EFFECT_SPLASH_LENGTH;
      ...
      model.getValue<number>(RoomObjectVariable.FIGURE_EFFECT) ===
      AvatarLogic.EFFECT_TYPE_SWIM
    ) {
      this._effectChangeTimeStamp =
        GetTickerTime() + AvatarLogic.EFFECT_SPLASH_LENGTH;
      ...
      model.getValue<number>(RoomObjectVariable.FIGURE_EFFECT) ===
      AvatarLogic.EFFECT_TYPE_SWIM_DARK
    ) {
      this._effectChangeTimeStamp =
        GetTickerTime() + AvatarLogic.EFFECT_SPLASH_LENGTH;
      this._newEffect = effect;
