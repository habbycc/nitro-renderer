...
export class PetLogic extends MovingObjectLogic {
  ...
  private updateModel(time: number, model: IRoomObjectModel): void {
    ...
    if (
      this._expressionEndTimestamp > 0 &&
      time > this._expressionEndTimestamp
    ) {
  ...
  public processUpdateMessage(message: RoomObjectUpdateMessage): void {
    ...
    if (message instanceof ObjectAvatarFigureUpdateMessage) {
      ...
      model.setValue(
        ...
        petFigureData.customLayerIds,
      );
      model.setValue(
        RoomObjectVariable.PET_CUSTOM_PARTS_IDS,
        petFigureData.customPartIds,
      );
      model.setValue(
      ...
      model.setValue(
        RoomObjectVariable.PET_IS_RIDING,
        message.isRiding ? 1 : 0,
      );
  ...
  public mouseEvent(
    ...
    switch (event.type) {
      ...
      case MouseEventType.MOUSE_DOWN: {
        const petType = this.object.model.getValue<number>(
          RoomObjectVariable.PET_TYPE,
        );
