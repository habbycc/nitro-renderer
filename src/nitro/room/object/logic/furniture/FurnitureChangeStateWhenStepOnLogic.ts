...
export class FurnitureChangeStateWhenStepOnLogic extends FurnitureLogic {
  constructor() {
    ...
    this.onRoomToObjectOwnAvatarMoveEvent =
      this.onRoomToObjectOwnAvatarMoveEvent.bind(this);
  }
  ...
  private onRoomToObjectOwnAvatarMoveEvent(
    event: RoomToObjectOwnAvatarMoveEvent,
  ): void {
    ...
    let sizeX = this.object.model.getValue<number>(
      RoomObjectVariable.FURNITURE_SIZE_X,
    );
    let sizeY = this.object.model.getValue<number>(
      RoomObjectVariable.FURNITURE_SIZE_Y,
    );

    const direction =
      ((Math.floor(this.object.getDirection().x) + 45) % 360) / 90;
