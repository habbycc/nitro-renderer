...
export class FurnitureRoomBackgroundColorLogic extends FurnitureMultiStateLogic {
  ...
  protected onDispose(): void {
    if (this._roomColorUpdated) {
      if (this.eventDispatcher && this.object) {
        const realRoomObject = this.object.model.getValue<number>(
  ...
  private processColorUpdate(): void {
    ...
    if (state > -1 && hue > -1 && saturation > -1 && lightness > -1) {
      ...
      this.object.model.setValue(
        RoomObjectVariable.FURNITURE_ROOM_BACKGROUND_COLOR_SATURATION,
        saturation,
      );
      this.object.model.setValue(
        RoomObjectVariable.FURNITURE_ROOM_BACKGROUND_COLOR_LIGHTNESS,
        lightness,
      );
