...
export class FurnitureRoomBrandingLogic extends FurnitureLogic {
  ...
  private processAdDataUpdateMessage(message: ObjectDataUpdateMessage): void {
    ...
    const state = parseInt(
      objectData.getValue(FurnitureRoomBrandingLogic.STATE),
    );

    if (!isNaN(state) && this.object.getState(0) !== state)
      this.object.setState(state, 0);

    const imageUrl = objectData.getValue(
      FurnitureRoomBrandingLogic.IMAGEURL_KEY,
    );
    const existingUrl = this.object.model.getValue<string>(
      RoomObjectVariable.FURNITURE_BRANDING_IMAGE_URL,
    );
    ...
    if (!existingUrl || existingUrl !== imageUrl) {
      ...
      this.object.model.setValue(
        RoomObjectVariable.FURNITURE_BRANDING_IMAGE_STATUS,
        0,
      );
    ...
    const clickUrl = objectData.getValue(
      FurnitureRoomBrandingLogic.CLICKURL_KEY,
    );

    if (clickUrl) {
      const existingUrl = this.object.model.getValue<string>(
        RoomObjectVariable.FURNITURE_BRANDING_URL,
      );
    ...
    const offsetX = parseInt(
      objectData.getValue(FurnitureRoomBrandingLogic.OFFSETX_KEY),
    );
    const offsetY = parseInt(
      objectData.getValue(FurnitureRoomBrandingLogic.OFFSETY_KEY),
    );
    const offsetZ = parseInt(
      objectData.getValue(FurnitureRoomBrandingLogic.OFFSETZ_KEY),
    );

    if (!isNaN(offsetX))
      this.object.model.setValue(
        RoomObjectVariable.FURNITURE_BRANDING_OFFSET_X,
        offsetX,
      );
    if (!isNaN(offsetY))
      this.object.model.setValue(
        RoomObjectVariable.FURNITURE_BRANDING_OFFSET_Y,
        offsetY,
      );
    if (!isNaN(offsetZ))
      this.object.model.setValue(
        RoomObjectVariable.FURNITURE_BRANDING_OFFSET_Z,
        offsetZ,
      );
    ...
    options =
      options + (FurnitureRoomBrandingLogic.OFFSETX_KEY + '=' + offsetX + '\t');
    options =
      options + (FurnitureRoomBrandingLogic.OFFSETY_KEY + '=' + offsetY + '\t');
    options =
      options + (FurnitureRoomBrandingLogic.OFFSETZ_KEY + '=' + offsetZ + '\t');
  ...
  private downloadBackground(): void {
    ...
    const imageUrl = model.getValue<string>(
      RoomObjectVariable.FURNITURE_BRANDING_IMAGE_URL,
    );
    const imageStatus = model.getValue<number>(
      RoomObjectVariable.FURNITURE_BRANDING_IMAGE_STATUS,
    );
    ...
    if (imageUrl.endsWith('.gif')) {
      this.object.model.setValue(
        RoomObjectVariable.FURNITURE_BRANDING_IS_ANIMATED,
        true,
      );
      ...
      fetch(imageUrl)
        ...
        .then((gif) => {
          ...
          Nitro.instance.roomEngine.roomContentLoader.createGifCollection(
            imageUrl,
            textures,
            durations,
          );
          ...
          this.processUpdateMessage(
            new ObjectAdUpdateMessage(ObjectAdUpdateMessage.IMAGE_LOADED),
          );
        })
        .catch((error) => {
          this.processUpdateMessage(
            new ObjectAdUpdateMessage(
              ObjectAdUpdateMessage.IMAGE_LOADING_FAILED,
            ),
          );
        });
    } else {
