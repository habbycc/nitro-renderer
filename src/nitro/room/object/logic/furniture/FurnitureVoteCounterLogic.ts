...
export class FurnitureVoteCounterLogic extends FurnitureMultiStateLogic {
  ...
  private updateTotal(k: number): void {
    ...
    if (this._total !== this.currentTotal) {
      ...
      if (
        difference * FurnitureVoteCounterLogic.UPDATE_INTERVAL >
        FurnitureVoteCounterLogic.MAX_UPDATE_TIME
      ) {
  ...
  public update(time: number): void {
    ...
    if (this.object) {
      if (
        this.currentTotal !== this._total &&
        time >= this._lastUpdate + this._interval
      ) {
  ...
  private get currentTotal(): number {
    return this.object.model.getValue<number>(
      RoomObjectVariable.FURNITURE_VOTE_COUNTER_COUNT,
    );
  }
}
