...
export class FurnitureLogic extends MovingObjectLogic {
  ...
  constructor() {
    ...
    if (FurnitureLogic.BOUNCING_STEPS === -1) {
      FurnitureLogic.BOUNCING_STEPS = NitroConfiguration.getValue<number>(
        'furni.rotation.bounce.steps',
        8,
      );
    }

    if (FurnitureLogic.BOUNCING_Z === -1) {
      FurnitureLogic.BOUNCING_Z = NitroConfiguration.getValue<number>(
        'furni.rotation.bounce.height',
        0.0625,
      );
    }
  }
  ...
  public getEventTypes(): string[] {
    ...
    if (this.widget)
      types.push(
        RoomObjectWidgetRequestEvent.OPEN_WIDGET,
        RoomObjectWidgetRequestEvent.CLOSE_WIDGET,
      );
  ...
  public processUpdateMessage(message: RoomObjectUpdateMessage): void {
      ...
      if (!(message instanceof ObjectMoveUpdateMessage)) {
        ...
        if (direction.x !== message.direction.x && this._directionInitialized) {
          if (
            ...
            this._storedRotateMessage = new RoomObjectUpdateMessage(
              message.location,
              message.direction,
            );
    ...
    if (message instanceof ObjectSelectedMessage) {
      if (this.contextMenu && this.eventDispatcher && this.object) {
        ...
        this.eventDispatcher.dispatchEvent(
          new RoomObjectWidgetRequestEvent(eventType, this.object),
        );
      }
    }
  ...
  private processObjectHeightUpdateMessage(
    message: ObjectHeightUpdateMessage,
  ): void {
    if (!message) return;
  ...
  private processItemDataUpdateMessage(
    message: ObjectItemDataUpdateMessage,
  ): void {
    if (!message) return;
      ...
      case MouseEventType.MOUSE_CLICK:
        if (this.eventDispatcher) {
          ...
          if (adUrl && adUrl.length)
            this.handleAdClick(this.object.id, this.object.type, adUrl);
        }
        return;
    ...
    if (this._bouncingStep <= FurnitureLogic.BOUNCING_STEPS / 2) {
      ...
      if (this._bouncingStep <= FurnitureLogic.BOUNCING_STEPS) {
        ...
        this._locationOffset.z =
          FurnitureLogic.BOUNCING_Z *
          (FurnitureLogic.BOUNCING_STEPS - this._bouncingStep);
      }
    }
  ...
  public useObject(): void {
    ...
    this.eventDispatcher.dispatchEvent(
      new RoomObjectStateChangedEvent(
        RoomObjectStateChangedEvent.STATE_CHANGE,
        this.object,
      ),
    );
