...
export class FurnitureScoreLogic extends FurnitureLogic {
  ...
  private updateScore(count: number): void {
    ...
    if (this._score !== currentScore) {
      ...
      if (
        difference * FurnitureScoreLogic.UPDATE_INTERVAL >
        FurnitureScoreLogic.MAX_UPDATE_TIME
      )
  ...
  public update(time: number): void {
    ...
    if (
      ...
      time >= this._scoreTimer + this._scoreIncreaser
    ) {
      ...
      if (_local_4 > _local_5 * (this._score - currentScore))
        _local_4 = _local_5 * (this._score - currentScore);
