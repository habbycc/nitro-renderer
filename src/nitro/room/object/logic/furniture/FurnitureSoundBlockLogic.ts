...
export class FurnitureSoundBlockLogic extends FurnitureMultiStateLogic {
  ...
  public initialize(asset: IAssetData): void {
    ...
    this.object.model.setValue(
      RoomObjectVariable.FURNITURE_SOUNDBLOCK_RELATIVE_ANIMATION_SPEED,
      1,
    );
  }
  ...
  public processUpdateMessage(message: RoomObjectUpdateMessage): void {
    ...
    if (message instanceof ObjectDataUpdateMessage)
      this.updateSoundBlockMessage(message);
  }
  ...
  private updateSoundBlockMessage(message: ObjectDataUpdateMessage): void {
    ...
    if (
      this._state === FurnitureSoundBlockLogic.STATE_UNINITIALIZED &&
      model.getValue<number>(RoomObjectVariable.FURNITURE_REAL_ROOM_OBJECT) ===
        1
    ) {
