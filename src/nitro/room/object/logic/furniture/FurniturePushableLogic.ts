...
export class FurniturePushableLogic extends FurnitureMultiStateLogic {
  ...
  public processUpdateMessage(message: RoomObjectUpdateMessage): void {
      ...
      if (difference) {
        if (Math.abs(difference.x) < 2 && Math.abs(difference.y) < 2) {
          ...
          if (Math.abs(difference.x) > 1 || Math.abs(difference.y) > 1) {
            prevLocation = Vector3d.sum(
              location,
              Vector3d.product(difference, 0.5),
            );
          }
    ...
    if (message.location && !isMoveMessage)
      super.processUpdateMessage(
        new ObjectMoveUpdateMessage(
          message.location,
          message.location,
    ...
    if (message instanceof ObjectDataUpdateMessage) {
      if (message.state > 0) {
        this.updateInterval =
          MovingObjectLogic.DEFAULT_UPDATE_INTERVAL /
          this.getUpdateIntervalValue(message.state);
      } else {
    ...
    if (isMoveMessage && message.isSlide)
      this.updateInterval = MovingObjectLogic.DEFAULT_UPDATE_INTERVAL;
  ...
  private handleDataUpdate(message: ObjectDataUpdateMessage): void {
    ...
    if (animation !== message.state) {
      ...
      message = new ObjectDataUpdateMessage(
        animation,
        legacyStuff,
        message.extra,
      );
  ...
  public update(time: number): void {
    ...
    if (Vector3d.dif(this.object.getLocation(), this._oldLocation).length !== 0)
      return;
