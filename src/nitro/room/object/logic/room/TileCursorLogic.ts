...
export class TileCursorLogic extends RoomObjectLogicBase {
  ...
  public processUpdateMessage(message: RoomObjectUpdateMessage): void {
    ...
    if (this._lastEventId && this._lastEventId === message.sourceEventId)
      return;
    ...
    if (this.object) {
      if (this._isHidden) {
        ...
        if (!message.visible) {
          ...
          this.object.model.setValue(
            RoomObjectVariable.TILE_CURSOR_HEIGHT,
            message.height,
          );

          this.object.setState(
            message.height > 0.8
              ? TileCursorLogic.CURSOR_HEIGHT_STATE
              : TileCursorLogic.CURSOR_VISIBLE_STATE,
          );
        }
      }
    }
