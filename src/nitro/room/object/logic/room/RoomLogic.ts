...
export class RoomLogic extends RoomObjectLogicBase {
  ...
  public initialize(roomMap: RoomMapData): void {
    ...
    this._skipColorTransition =
      NitroConfiguration.getValue<boolean>('room.color.skip.transition') ===
      true;
  }
  ...
  private updateBackgroundColor(k: number): void {
    ...
    if (k - this._colorChangedTime >= this._colorTransitionLength) {
      ...
      const _local_13 =
        (k - this._colorChangedTime) / this._colorTransitionLength;
      ...
      newColor =
        this._originalLight +
        (this._targetLight - this._originalLight) * _local_13;
  ...
  private onObjectRoomUpdateMessage(
    message: ObjectRoomUpdateMessage,
  ...
  private onObjectRoomMaskUpdateMessage(
    message: ObjectRoomMaskUpdateMessage,
    _arg_2: IRoomObjectModel,
  ): void {
  ...
  private onObjectRoomPlaneVisibilityUpdateMessage(
    message: ObjectRoomPlaneVisibilityUpdateMessage,
    model: IRoomObjectModel,
  ): void {
    let visible = 0;
  ...
  private onObjectRoomPlanePropertyUpdateMessage(
    message: ObjectRoomPlanePropertyUpdateMessage,
    model: IRoomObjectModel,
  ): void {
    switch (message.type) {
  ...
  private onObjectRoomFloorHoleUpdateMessage(
    message: ObjectRoomFloorHoleUpdateMessage,
    model: IRoomObjectModel,
  ): void {
    switch (message.type) {
      case ObjectRoomFloorHoleUpdateMessage.ADD:
        this._planeParser.addFloorHole(
          message.id,
          message.x,
          message.y,
          message.width,
  ...
  private onObjectRoomColorUpdateMessage(
    message: ObjectRoomColorUpdateMessage,
    model: IRoomObjectModel,
  ): void {
    ...
    model.setValue(
      RoomObjectVariable.ROOM_COLORIZE_BG_ONLY,
      message.backgroundOnly,
    );
  }

  private onObjectRoomMapUpdateMessage(
    message: ObjectRoomMapUpdateMessage,
  ): void {
  ...
  public mouseEvent(
    ...
    const planeNormalDirection =
      this._planeParser.getPlaneNormalDirection(planeId);
    ...
    planePosition = geometry.getPlanePosition(
      screenPoint,
      planeLocation,
      planeLeftSide,
      planeRightSide,
    ...
    const _local_18 = Vector3d.product(
      planeLeftSide,
      planePosition.x / leftSideLength,
    );

    _local_18.add(
      Vector3d.product(planeRightSide, planePosition.y / rightSideLength),
    );
    ...
    if (
      ...
      this.object.model.setValue(
        RoomObjectVariable.ROOM_SELECTED_PLANE,
        planeId + 1,
      );
    } else {
      this.object.model.setValue(RoomObjectVariable.ROOM_SELECTED_PLANE, 0);
    ...
    if (
      ...
      event.type === MouseEventType.ROLL_OVER
    )
      eventType = RoomObjectMouseEvent.MOUSE_MOVE;
    else if (event.type === MouseEventType.MOUSE_CLICK)
    ...
    switch (event.type) {
      ...
      case MouseEventType.MOUSE_CLICK: {
        ...
        if (planeType === RoomPlaneData.PLANE_FLOOR) {
          ...
          planeType === RoomPlaneData.PLANE_WALL ||
          planeType === RoomPlaneData.PLANE_LANDSCAPE
        ) {
          let direction = 90;
