  ...
  implements IAvatarImageListener, IAvatarEffectListener
{
  ...
  public update(
    ...
    if (update1 || update2) {
      ...
      if (!_local_20 || _local_20.length < 3)
        _local_20 = AvatarVisualization.DEFAULT_CANVAS_OFFSETS;

      const sprite = this.getSprite(AvatarVisualization.SPRITE_INDEX_AVATAR);

      if (sprite) {
        const highlightEnabled =
          this.object.model.getValue<number>(
            RoomObjectVariable.FIGURE_HIGHLIGHT_ENABLE,
          ) === 1 &&
          this.object.model.getValue<number>(
            RoomObjectVariable.FIGURE_HIGHLIGHT,
          ) === 1;

        const avatarImage = this._avatarImage.getImage(
          AvatarSetType.FULL,
          highlightEnabled,
        );
        ...
        if (this._isLaying) {
          if (this._layInside) sprite.relativeDepth = -0.5;
            ...
            sprite.relativeDepth =
              AvatarVisualization.AVATAR_SPRITE_LAYING_DEPTH + _local_20[2];
        } else {
          sprite.relativeDepth =
            AvatarVisualization.AVATAR_SPRITE_DEFAULT_DEPTH + _local_20[2];
        }
      ...
      const typingBubble = this.getAddition(
        AvatarVisualization.TYPING_BUBBLE_ID,
      ) as TypingBubbleAddition;

      if (typingBubble) {
        if (!this._isLaying)
          typingBubble.relativeDepth =
            AvatarVisualization.AVATAR_SPRITE_DEFAULT_DEPTH -
            0.01 +
            _local_20[2];
        else
          typingBubble.relativeDepth =
            AvatarVisualization.AVATAR_SPRITE_LAYING_DEPTH -
            0.01 +
            _local_20[2];
      }
      ...
      for (const spriteData of this._avatarImage.getSprites()) {
        if (spriteData.id === AvatarVisualization.AVATAR) {
          const sprite = this.getSprite(
            AvatarVisualization.SPRITE_INDEX_AVATAR,
          );
          ...
          if (sprite) {
            ...
            if (spriteData.hasStaticY) {
              sprite.offsetY +=
                (this._verticalOffset * scale) /
                (2 * AvatarVisualization.BASE_Y_SCALE);
            } else {
            ...
            if (this._isLaying) {
              sprite.relativeDepth =
                AvatarVisualization.AVATAR_SPRITE_LAYING_DEPTH -
                0.001 * this.totalSprites * offsetZ;
            } else {
              sprite.relativeDepth =
                AvatarVisualization.AVATAR_SPRITE_DEFAULT_DEPTH -
                0.001 * this.totalSprites * offsetZ;
  ...
  private createAvatarImage(scale: number, effectId: number): IAvatarImage {
    ...
    if (!cachedImage) {
      cachedImage = this._data.createAvatarImage(
        this._figure,
        scale,
        this._gender,
        this,
      ...
      if (cachedImage) {
        if (!effectId) {
          ...
          if (
            this._cachedAvatarEffects.length >=
            AvatarVisualization.MAX_EFFECT_CACHE
          ) {
            const cached = this._cachedAvatarEffects.remove(
              this._cachedAvatarEffects.getKey(0),
            );
  ...
  protected updateModel(model: IRoomObjectModel, scale: number): boolean {
    ...
    const expression = model.getValue<number>(
    ...
    const gesture =
      model.getValue<number>(RoomObjectVariable.FIGURE_GESTURE) || 0;
    ...
    const postureParameter = model.getValue<string>(
      RoomObjectVariable.FIGURE_POSTURE_PARAMETER,
    );
    ...
    const canStandUp = model.getValue<boolean>(
      RoomObjectVariable.FIGURE_CAN_STAND_UP,
    );
    ...
    const verticalOffset =
      model.getValue<number>(RoomObjectVariable.FIGURE_VERTICAL_OFFSET) *
      AvatarVisualization.BASE_Y_SCALE;
    ...
    const effect =
      model.getValue<number>(RoomObjectVariable.FIGURE_EFFECT) || 0;
    ...
    const carryObject =
      model.getValue<number>(RoomObjectVariable.FIGURE_CARRY_OBJECT) || 0;
    ...
    const useObject =
      model.getValue<number>(RoomObjectVariable.FIGURE_USE_OBJECT) || 0;
    ...
    const headDirection = model.getValue<number>(
    ...
    if (this._sleep) {
      if (!idleAddition)
        idleAddition = this.addAddition(
          new FloatingIdleZAddition(
            AvatarVisualization.FLOATING_IDLE_Z_ID,
            this,
          ),
        );
    ...
    if (isMuted) {
      if (!mutedAddition)
        mutedAddition = this.addAddition(
          new MutedBubbleAddition(AvatarVisualization.MUTED_BUBBLE_ID, this),
        );
      ...
      const isTyping =
        model.getValue<number>(RoomObjectVariable.FIGURE_IS_TYPING) > 0;

      let typingAddition = this.getAddition(
        AvatarVisualization.TYPING_BUBBLE_ID,
      );
    ...
    const guideStatusValue =
      model.getValue<number>(RoomObjectVariable.FIGURE_GUIDE_STATUS) || 0;

    if (guideStatusValue !== AvatarGuideStatus.NONE) {
      this.removeAddition(AvatarVisualization.GUIDE_BUBBLE_ID);
      this.addAddition(
        new GuideStatusBubbleAddition(
          AvatarVisualization.GUIDE_BUBBLE_ID,
          this,
          guideStatusValue,
        ),
      );
    ...
    const isPlayingGame =
      model.getValue<number>(RoomObjectVariable.FIGURE_IS_PLAYING_GAME) > 0;

    let gameClickAddition = this.getAddition(
      AvatarVisualization.GAME_CLICK_TARGET_ID,
    );
    ...
    if (isPlayingGame) {
      ...
      if (gameClickAddition)
        this.removeAddition(AvatarVisualization.GAME_CLICK_TARGET_ID);
    }

    const numberValue = model.getValue<number>(
    ...
    if (numberValue > 0) {
      if (!numberAddition)
        numberAddition = this.addAddition(
          new NumberBubbleAddition(
            AvatarVisualization.NUMBER_BUBBLE_ID,
            numberValue,
            this,
          ),
        );
      ...
      if (numberAddition)
        this.removeAddition(AvatarVisualization.NUMBER_BUBBLE_ID);
    }

    let expressionAddition = this.getAddition(
    ...
    if (this.updateFigure(model.getValue<string>(RoomObjectVariable.FIGURE)))
      needsUpdate = true;
    ...
    const highlightEnabled =
      model.getValue<number>(RoomObjectVariable.FIGURE_HIGHLIGHT_ENABLE) > 0;
    ...
    if (this._highlightEnabled) {
      const highlight =
        model.getValue<number>(RoomObjectVariable.FIGURE_HIGHLIGHT) > 0;
  ...
  private processActionsForAvatar(avatar: IAvatarImage): void {
    ...
    if (this._gesture > 0)
      this._avatarImage.appendAction(
        AvatarAction.GESTURE,
        AvatarAction.getGesture(this._gesture),
      );

    if (this._dance > 0)
      this._avatarImage.appendAction(AvatarAction.DANCE, this._dance);

    if (this._sign > -1)
      this._avatarImage.appendAction(AvatarAction.SIGN, this._sign);

    if (this._carryObject > 0)
      this._avatarImage.appendAction(
        AvatarAction.CARRY_OBJECT,
        this._carryObject,
      );

    if (this._useObject > 0)
      this._avatarImage.appendAction(AvatarAction.USE_OBJECT, this._useObject);
    ...
    if (this._effect > 0)
      this._avatarImage.appendAction(AvatarAction.EFFECT, this._effect);
  ...
  private clearAvatar(): void {
    for (const avatar of this._cachedAvatars.getValues())
      avatar && avatar.dispose();

    for (const avatar of this._cachedAvatarEffects.getValues())
      avatar && avatar.dispose();
  ...
  private updateShadow(scale: number): void {
    ...
    let hasShadow =
      this._posture === 'mv' ||
      this._posture === 'std' ||
      (this._posture === 'sit' && this._canStandUp);

    if (this._effect === AvatarVisualization.SNOWBOARDING_EFFECT)
      hasShadow = false;
