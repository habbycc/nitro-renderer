...
export class FloatingHeartAddition extends ExpressionAddition {
  ...
  public update(sprite: IRoomObjectSprite, scale: number): void {
    ...
    if (scale < 48) {
      this._asset = this.visualization.getAvatarRenderAsset(
        'avatar_addition_user_blowkiss_small',
      );
  ...
  public animate(sprite: IRoomObjectSprite): boolean {
    ...
    if (this._state === FloatingHeartAddition.STATE_DELAY) {
      if (
        GetTickerTime() - this._startTime <
        FloatingHeartAddition.DELAY_BEFORE_ANIMATION
      )
        return false;
