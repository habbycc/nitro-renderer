...
export class FloatingIdleZAddition implements IAvatarAddition {
  ...
  private getSpriteAssetName(state: number): string {
    ...
    return (
      'avatar_addition_user_idle_' +
      side +
      '_' +
      state +
      (this._scale < 48 ? '_small' : '')
    );
  }
  ...
  public update(sprite: IRoomObjectSprite, scale: number): void {
    ...
    this._asset = this._visualization.getAvatarRenderAsset(
      this.getSpriteAssetName(
        this._state === FloatingIdleZAddition.STATE_FRAME_A ? 1 : 2,
      ),
    );
  ...
  public animate(sprite: IRoomObjectSprite): boolean {
    ...
    if (this._state === FloatingIdleZAddition.STATE_DELAY) {
      if (
        totalTimeRunning - this._startTime >=
        FloatingIdleZAddition.DELAY_BEFORE_ANIMATION
      ) {
        this._state = FloatingIdleZAddition.STATE_FRAME_A;
        this._startTime = totalTimeRunning;
        this._asset = this._visualization.getAvatarRenderAsset(
          this.getSpriteAssetName(1),
        );
      }
    }
    ...
    if (this._state === FloatingIdleZAddition.STATE_FRAME_A) {
      if (
        totalTimeRunning - this._startTime >=
        FloatingIdleZAddition.DELAY_PER_FRAME
      ) {
        this._state = FloatingIdleZAddition.STATE_FRAME_B;
        this._startTime = totalTimeRunning;
        this._asset = this._visualization.getAvatarRenderAsset(
          this.getSpriteAssetName(2),
        );
      }
    }
