...
export class FurnitureVisualization extends RoomObjectSpriteVisualization {
  ...
  protected resetLayers(scale: number, direction: number): void {
    ...
    this.setLayerCount(
      ((this._data && this._data.getLayerCount(scale)) || 0) +
        this.getAdditionalLayerCount(),
    );
  }
  ...
  protected updateObject(scale: number, direction: number): boolean {
    ...
    if (this._data) {
      const validDirection = this._data.getValidDirection(
        scale,
        offsetDirection,
      );
  ...
  protected updateModel(scale: number): boolean {
    ...
    this._selectedColor = model.getValue<number>(
      RoomObjectVariable.FURNITURE_COLOR,
    );
    this._clickUrl = model.getValue<string>(
      RoomObjectVariable.FURNITURE_AD_URL,
    );
    this._clickHandling =
      (this._clickUrl &&
        this._clickUrl !== '' &&
        this._clickUrl.indexOf('http') === 0) ||
      false;
    this._furnitureLift =
      model.getValue<number>(RoomObjectVariable.FURNITURE_LIFT_AMOUNT) || 0;

    let alphaMultiplier = model.getValue<number>(
    ...
    if (assetName && sprite) {
      ...
      if (assetData && assetData.texture) {
        ...
        if (layerId !== this._shadowLayerIndex) {
          ...
          sprite.color = this.getLayerColor(
            scale,
            layerId,
            this._selectedColor,
          );
          sprite.offsetX =
            assetData.offsetX +
            this.getLayerXOffset(scale, this._direction, layerId);
          sprite.offsetY =
            assetData.offsetY +
            this.getLayerYOffset(scale, this._direction, layerId);
          sprite.blendMode = this.getLayerInk(scale, this._direction, layerId);
          sprite.alphaTolerance = this.getLayerIgnoreMouse(
            scale,
            this._direction,
            layerId,
          )
        ...
        sprite.relativeDepth =
          relativeDepth * FurnitureVisualization.DEPTH_MULTIPLIER;
        sprite.name = assetName;
        sprite.libraryAssetName = this.getLibraryAssetNameForSprite(
          assetData,
          sprite,
        );
  ...
  protected getLibraryAssetNameForSprite(
    asset: IGraphicAsset,
  ...
  protected getSpriteAssetName(scale: number, layerId: number): string {
    if (
      !this._data ||
      layerId >= FurnitureVisualizationData.LAYER_LETTERS.length
    )
      return '';
  ...
  protected cacheSpriteAssetName(
    scale: number,
    layerId: number,
    cache: boolean,
  ): string {
    ...
    const assetName =
      this._type +
      (isntIcon
        ? '_' + size + '_' + layerCode + '_' + this._direction + '_'
        : '_icon_' + layerCode);
  ...
  protected getLayerIgnoreMouse(
    ...
    const ignoreMouse = this._data.getLayerIgnoreMouse(
      scale,
      direction,
      layerId,
    );
