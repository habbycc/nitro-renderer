...
export class FurnitureValRandomizerVisualization extends FurnitureAnimatedVisualization {
  ...
  protected setAnimation(animationId: number): void {
    if (animationId === 0) {
      if (!this._running) {
        ...
        this._stateQueue.push(
          FurnitureValRandomizerVisualization.ANIMATION_ID_START_ROLL,
        );
        this._stateQueue.push(
          FurnitureValRandomizerVisualization.ANIMATION_ID_ROLL,
        );
    ...
    if (
      animationId > 0 &&
      animationId <=
        FurnitureValRandomizerVisualization.ANIMATION_ID_OFFSET_SLOW2
    ) {
      if (this._running) {
        this._running = false;
        this._stateQueue = [];
        ...
        if (this.direction === 2) {
          this._stateQueue.push(
            FurnitureValRandomizerVisualization.ANIMATION_ID_OFFSET_SLOW1 + 5,
          );
          this._stateQueue.push(
            FurnitureValRandomizerVisualization.ANIMATION_ID_OFFSET_SLOW2 + 5,
          );
        } else {
          this._stateQueue.push(
            FurnitureValRandomizerVisualization.ANIMATION_ID_OFFSET_SLOW1 +
              animationId,
          );
          this._stateQueue.push(
            FurnitureValRandomizerVisualization.ANIMATION_ID_OFFSET_SLOW2 +
        ...
        this._stateQueue.push(
          FurnitureValRandomizerVisualization.ANIMATION_ID_OFF,
        );
