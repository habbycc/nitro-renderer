...
export class FurnitureParticleSystem {
  ...
  public getLayerYOffset(
    scale: number,
    direction: number,
    layerId: number,
  ): number {
    if (
      this._currentEmitter &&
      this._currentEmitter.roomObjectSpriteId === layerId
    ) {
  ...
  public updateSprites(): void {
    ...
    if (this._hasIgnited) {
      if (this._currentEmitter.roomObjectSpriteId >= 0)
        this._visualization.getSprite(
          this._currentEmitter.roomObjectSpriteId,
        ).visible = false;
    }
  }
  ...
  public updateAnimation(): void {
    ...
    if (this._hasIgnited) {
      ...
      for (const particle of this._currentEmitter.particles) {
        const tx =
          this._centerX +
          (((particle.x - particle.z) * k) / 10) * this._scaleMultiplier;
        const ty =
          this._centerY -
          offsetY +
          (((particle.y + (particle.x + particle.z) / 2) * k) / 10) *
        ...
        if (asset && asset.texture) {
          if (particle.fade && particle.alphaMultiplier < 1) {
            ...
            this._translationMatrix.translate(
              tx + asset.offsetX,
              ty + asset.offsetY,
            );
  ...
  public parseData(particleSystem: IParticleSystem): void {
    ...
    this._canvasId =
      particleSystem.canvasId !== undefined ? particleSystem.canvasId : -1;
    this._offsetY =
      particleSystem.offsetY !== undefined ? particleSystem.offsetY : 10;
    ...
    const bgColor =
      particleSystem.bgColor !== undefined ? particleSystem.bgColor : '0';
    ...
    for (const emitter of particleSystem.emitters) {
      ...
      const particleEmitter = new FurnitureParticleSystemEmitter(
        emitterName,
        emitterSpriteId,
      );
      ...
      const burstPulse =
        emitter.burstPulse !== undefined ? emitter.burstPulse : 1;
      ...
      for (const particle of emitter.particles) {
        ...
        for (const name of particle.frames)
          frames.push(this._visualization.asset.getAsset(name));
  ...
  public _Str_17988(particleSystem: FurnitureParticleSystem): void {
    ...
    if (particleSystem._emitters && particleSystem._currentEmitter) {
      emitterId = particleSystem._emitters.getKey(
        particleSystem._emitters
          .getValues()
          .indexOf(particleSystem._currentEmitter),
      );
    }
    ...
    if (this._currentEmitter)
      this._currentEmitter.copyStateFrom(
        particleSystem._currentEmitter,
        particleSystem._size / this._size,
      );
