...
export class FurnitureBadgeDisplayVisualization extends FurnitureAnimatedVisualization {
  ...
  protected updateModel(scale: number): boolean {
    ...
    const badgeStatus = this.object.model.getValue<number>(
      RoomObjectVariable.FURNITURE_BADGE_IMAGE_STATUS,
    );
    const badgeId = this.object.model.getValue<string>(
      RoomObjectVariable.FURNITURE_BADGE_ASSET_NAME,
    );
  ...
  protected getLayerXOffset(
    ...
    if (
      this.getLayerTag(scale, direction, layerId) ===
      FurnitureBadgeDisplayVisualization.BADGE
    ) {
