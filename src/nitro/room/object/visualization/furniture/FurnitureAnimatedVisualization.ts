...
export class FurnitureAnimatedVisualization extends FurnitureVisualization {
  ...
  protected getAnimationId(animationData: AnimationStateData): number {
    if (
      this.animationId !==
        FurnitureAnimatedVisualization.DEFAULT_ANIMATION_ID &&
      this._data.hasAnimation(this._animationScale, this.animationId)
    )
      ...
      if (state !== this._state) {
        ...
        this._animationChangeTime =
          this.object.model.getValue<number>(
            RoomObjectVariable.FURNITURE_STATE_UPDATE_TIME,
          ) || 0;
      }
  ...
  protected updateModel(scale: number): boolean {
    if (super.updateModel(scale)) {
      if (this.usesAnimationResetting()) {
        const updateTime = this.object.model.getValue<number>(
          RoomObjectVariable.FURNITURE_STATE_UPDATE_TIME,
        );
      ...
      const state = this.object.model.getValue<number>(
        RoomObjectVariable.FURNITURE_AUTOMATIC_STATE_INDEX,
      );

      if (!isNaN(state)) {
        const animationId = this._data.getAnimationId(
          this._animationScale,
          state,
        );
    ...
    if (_arg_3) {
      ...
      if (animationId !== state) {
        ...
        if (
          ...
          const transition =
            AnimationData.getTransitionToAnimationId(animationId);
          ...
          if (this.usesAnimationResetting()) {
            const transition =
              AnimationData.getTransitionFromAnimationId(state);
  ...
  protected updateAnimations(scale: number): number {
    ...
    if (this._animationData.animationOver) {
      if (
        AnimationData.isTransitionFromAnimation(
  ...
  protected updateFramesForAnimation(
    ...
    if (!frameCount)
      frameCount = this._data.getStartFrame(
        scale,
        animationId,
        this._direction,
      );
    ...
    while (layerId >= 0) {
      ...
      if (!animationPlayed || this._directionChanged) {
        ...
        if (frame) {
          if (
            frame.isLastFrame &&
            frame.remainingFrameRepeats <= this.frameIncrease
          ) {
        ...
        if (
          ...
          (frame.remainingFrameRepeats >= 0 &&
            (frame.remainingFrameRepeats =
              frame.remainingFrameRepeats - this.frameIncrease) <= 0)
        ) {
          ...
          if (sequenceId === AnimationFrame.SEQUENCE_NOT_DEFINED) {
            ...
            frame = this._data.getFrameFromSequence(
              scale,
              animationId,
              this._direction,
              layerId,
        ...
        if (
          !frame ||
          frame.remainingFrameRepeats == AnimationFrame.FRAME_REPEAT_FOREVER
        ) {
          lastFramePlayed = true;
