...
export class FurnitureHabboWheelVisualization extends FurnitureAnimatedVisualization {
  ...
  protected setAnimation(animationId: number): void {
    if (animationId === -1) {
      if (!this._running) {
        ...
        this._stateQueue.push(
          FurnitureHabboWheelVisualization.ANIMATION_ID_START_ROLL,
        );
        this._stateQueue.push(
          FurnitureHabboWheelVisualization.ANIMATION_ID_ROLL,
        );
    ...
    if (
      animationId > 0 &&
      animationId <= FurnitureHabboWheelVisualization.ANIMATION_ID_OFFSET_SLOW1
    ) {
      if (this._running) {
        this._running = false;
        this._stateQueue = [];
        ...
        this._stateQueue.push(
          FurnitureHabboWheelVisualization.ANIMATION_ID_OFFSET_SLOW1 +
            animationId,
        );
        this._stateQueue.push(
          FurnitureHabboWheelVisualization.ANIMATION_ID_OFFSET_SLOW2 +
            animationId,
        );
  ...
  protected updateAnimation(scale: number): number {
    if (
      this.getLastFramePlayed(1) &&
      this.getLastFramePlayed(2) &&
      this.getLastFramePlayed(3)
    ) {
