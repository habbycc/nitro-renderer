...
export class FurnitureGiftWrappedFireworksVisualization extends FurnitureFireworksVisualization {
  ...
  private updatePresentWrap(): void {
    ...
    const local3 = 1000;
    const extras = this.object.model.getValue<string>(
    ...
    this._packetType =
      packetType >
      FurnitureGiftWrappedFireworksVisualization.MAX_PACKET_TYPE_VALUE
        ? 0
        : packetType;
    this._ribbonType =
      ribbonType >
      FurnitureGiftWrappedFireworksVisualization.MAX_RIBBON_TYPE_VALUE
        ? 0
        : ribbonType;
  }
  ...
  public getFrameNumber(scale: number, layerId: number): number {
    if (
      ...
      FurnitureGiftWrappedFireworksVisualization.PRESENT_DEFAULT_STATE
    ) {
  ...
  public getSpriteAssetName(scale: number, layerId: number): string {
    ...
    assetName =
      assetName + ('_' + size + '_' + layerCode + '_' + this.direction);
