...
export class FurnitureMannequinVisualization
  extends FurnitureVisualization
  implements IAvatarImageListener
{
  ...
  private updateAvatar(forceUpdate: boolean = false): void {
    if (!this.avatarExists() || forceUpdate) {
      const avatarImage = this._data.createAvatarImage(
        this._figure,
        this._mannequinScale,
        this._gender,
        this,
      );
      ...
      if (avatarImage) {
        ...
        this.asset.addAsset(
          ...
          avatarImage.getImage(AvatarSetType.FULL, false, 1, false),
          true,
        );
  ...
  protected getSpriteAssetName(scale: number, layerId: number): string {
    ...
    if (
      this._figure &&
      tag === FurnitureMannequinVisualization.AVATAR_IMAGE_SPRITE_TAG &&
      this.avatarExists()
    ) {
