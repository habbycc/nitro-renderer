...
export class FurnitureVoteMajorityVisualization extends FurnitureAnimatedVisualization {
  ...
  protected getFrameNumber(scale: number, layerId: number): number {
    const result = this.object.model.getValue<number>(
      RoomObjectVariable.FURNITURE_VOTE_MAJORITY_RESULT,
    );
  ...
  protected getLayerAlpha(
    ...
    if (
      !(
        FurnitureVoteMajorityVisualization.HIDE_RESULTS_STATES.indexOf(
          this.object.getState(0),
        ) === -1
      ) ||
