...
export class FurnitureFireworksVisualization extends FurnitureAnimatedVisualization {
  ...
  public dispose(): void {
    ...
    if (this._particleSystems) {
      for (const particleSystem of this._particleSystems.getValues())
  ...
  protected updateObject(scale: number, direction: number): boolean {
    if (super.updateObject(scale, direction)) {
      if (!this._particleSystems) {
        ...
        if (this._particleSystems)
          this._currentParticleSystem = this._particleSystems.getValue(scale);
        else
          NitroLogger.log(
            'ERROR Particle systems could not be read!',
            this.object.type,
          );
      } else {
        if (
          scale !== this._scale ||
          this._particleSystems.getValue(scale) !== this._currentParticleSystem
        ) {
  ...
  protected setAnimation(id: number): void {
    if (this._currentParticleSystem)
      this._currentParticleSystem.setAnimation(id);
  ...
  protected getLayerYOffset(
    ...
    if (
      this._currentParticleSystem &&
      this._currentParticleSystem.controlsSprite(layerId)
    ) {
      return this._currentParticleSystem.getLayerYOffset(
        scale,
        direction,
        layerId,
      );
