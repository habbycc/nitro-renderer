...
export class FurnitureBrandedImageVisualization extends FurnitureVisualization {
  ...
  protected updateModel(scale: number): boolean {
    ...
    if (flag) {
      ...
      this._isAnimated =
        this.object.model.getValue<boolean>(
  ...
  private checkIfImageChanged(): boolean {
    const imageUrl = this.object.model.getValue<string>(
  ...
  protected checkIfImageReady(): boolean {
    ...
    const imageStatus = this.object.model.getValue<number>(
    ...
    if (imageStatus === 1) {
      ...
      if (this._isAnimated) {
        const gifCollection =
          Nitro.instance.roomEngine.roomContentLoader.getGifCollection(
            imageUrl,
          );
  ...
  protected buildAssetsForGif(): void {
    ...
    if (
      !textures.length ||
      !durations.length ||
      textures.length !== durations.length
    )
      return;
  ...
  protected getSpriteAssetName(scale: number, layerId: number): string {
    ...
    if (
      tag === FurnitureBrandedImageVisualization.BRANDED_IMAGE &&
      this._imageUrl
    ) {
