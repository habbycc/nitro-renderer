...
export class FurnitureQueueTileVisualization extends FurnitureAnimatedVisualization {
  ...
  protected setAnimation(animationId: number): void {
    if (
      animationId === FurnitureQueueTileVisualization.ANIMATION_ID_ROLL_ONCE
    ) {
      this._stateQueue = [];
      this._stateQueue.push(
        FurnitureQueueTileVisualization.ANIMATION_ID_NORMAL,
      );

      this._animationCounter =
        FurnitureQueueTileVisualization.ANIMATION_DURATION;
    }
