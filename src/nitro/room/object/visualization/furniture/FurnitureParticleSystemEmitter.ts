...
export class FurnitureParticleSystemEmitter extends FurnitureParticleSystemParticle {
  ...
  public copyStateFrom(
    emitter: FurnitureParticleSystemEmitter,
    scale: number,
  ): void {
  ...
  private releaseParticles(
    particle: FurnitureParticleSystemParticle,
    ...
    while (i < this._particlesPerFrame) {
      switch (this._explosionShape) {
        case FurnitureParticleSystemEmitter.CONE:
          newDirection.x = this.randomBoolean(0.5)
            ? Math.random()
            : -Math.random();
          newDirection.y = -(Math.random() + 1);
          newDirection.z = this.randomBoolean(0.5)
            ? Math.random()
            : -Math.random();
          break;
        ...
        case FurnitureParticleSystemEmitter.SPHERE:
          newDirection.x = this.randomBoolean(0.5)
            ? Math.random()
            : -Math.random();
          newDirection.y = this.randomBoolean(0.5)
            ? Math.random()
            : -Math.random();
          newDirection.z = this.randomBoolean(0.5)
            ? Math.random()
  ...
  private getRandomParticleConfiguration(): ParticleSystemParticle {
    const index: number = Math.trunc(
      Math.floor(Math.random() * this._particleConfigurations.length),
    );
  ...
  public verlet(): void {
    if (this.isAlive || this._emittedParticles < this._maxNumberOfParticles) {
      ...
      this.x =
        (2 - this._airFriction) * this.x - (1 - this._airFriction) * this.lastX;
      this.y =
        (2 - this._airFriction) * this.y -
        (1 - this._airFriction) * this.lastY +
        this._gravity * this._timeStep * this._timeStep;
      this.z =
        (2 - this._airFriction) * this.z - (1 - this._airFriction) * this.lastZ;
      this.lastX = x;
      this.lastY = y;
      this.lastZ = z;
    }
    ...
    for (const particle of this._particles) {
      ...
      const z = particle.z;
      particle.x =
        (2 - this._airFriction) * particle.x -
        (1 - this._airFriction) * particle.lastX;
      particle.y =
        (2 - this._airFriction) * particle.y -
        (1 - this._airFriction) * particle.lastY +
        this._gravity * this._timeStep * this._timeStep;
      particle.z =
        (2 - this._airFriction) * particle.z -
        (1 - this._airFriction) * particle.lastZ;
      particle.lastX = x;
      particle.lastY = y;
      particle.lastZ = z;
