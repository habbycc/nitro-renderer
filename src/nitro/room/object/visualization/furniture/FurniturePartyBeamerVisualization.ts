...
export class FurniturePartyBeamerVisualization extends FurnitureAnimatedVisualization {
  ...
  private getNewPoint(scale: number, layerId: number): NitroPoint {
    ...
    const _local_9: number =
      animationPhase + animationDirection * animationSpeed;
    ...
    const _local_10: number =
      (diameter - Math.abs(animationPhase)) * animationFactor;

    let _local_11: number =
      animationDirection * Math.sin(Math.abs(animationPhase / 4)) * _local_10;
    ...
    animationPhase =
      animationPhase + animationDirection * animationSpeed * _local_7;

    this._animPhaseIndex[layerId] = animationPhase;

    if (Math.trunc(_local_11) == 0)
      this._animFactorIndex[layerId] = this.getRandomAmplitudeFactor();
