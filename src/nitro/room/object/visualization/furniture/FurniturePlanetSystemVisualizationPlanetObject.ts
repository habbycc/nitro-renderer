...
export class FurniturePlanetSystemVisualizationPlanetObject {
  ...
  public update(
    ...
    this._position =
      this._position +
      this._arcSpeed /
        FurniturePlanetSystemVisualizationPlanetObject.SYSTEM_TEMPO;

    offsets[this._index] = this.getPositionVector(rootPosition, scale);

    for (const child of this._children)
      child.update(offsets, this._positionVector, scale);
  }
  ...
  public getPositionVector(position: Vector3d, scale: number): Vector3d {
    ...
    this._positionVector.y =
      (sine + cos) * (scale / 2) * 0.5 - this._height * (scale / 2);
  ...
  public addChild(
    planetObject: FurniturePlanetSystemVisualizationPlanetObject,
  ): void {
