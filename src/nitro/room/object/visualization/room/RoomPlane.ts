...
export class RoomPlane implements IRoomPlane {
  ...
  private needsNewTexture(k: IRoomGeometry, _arg_2: number): boolean {
    ...
    if (
      this._canBeVisible &&
      (!planeBitmap ||
        (planeBitmap.timeStamp >= 0 && _arg_2 > planeBitmap.timeStamp) ||
        this._maskChanged)
    )
      return true;
          ...
          if (assetName) {
            ...
            if (asset) {
              ...
              const _local_11 = new Point(
                _local_3 + asset.offsetX,
                _local_4 + asset.offsetY,
              );
      ...
      while (i < this._bitmapMasks.length) {
        ...
        if (mask) {
          ...
          this._bitmapMasksOld.push(
            new RoomPlaneBitmapMask(type, mask.leftSideLoc, mask.rightSideLoc),
          );
        }
      ...
      while (i < this._rectangleMasks.length) {
        ...
        if (rectMask) {
          ...
          const wd =
            ...
            this._leftSide.length;
          const ht =
            (this._maskBitmapData.height * rectMask.rightSideLength) /
            this._rightSide.length;
