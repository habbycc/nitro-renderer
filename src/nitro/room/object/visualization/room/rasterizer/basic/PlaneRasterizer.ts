...
export class PlaneRasterizer implements IPlaneRasterizer {
  ...
  private initializeTexturesAndMaterials(): void {
    if (this._data.textures && this._data.textures.length)
      this.parseTextures(this._data.textures, this.assetCollection);
    ...
    for (const material of materials) {
      ...
      if (material.matrices && material.matrices.length) {
        for (const matrix of material.matrices) {
          ...
          const normalMinX =
            PlaneMaterialCellMatrix.MIN_NORMAL_COORDINATE_VALUE;
          const normalMaxX =
            PlaneMaterialCellMatrix.MAX_NORMAL_COORDINATE_VALUE;
          const normalMinY =
            PlaneMaterialCellMatrix.MIN_NORMAL_COORDINATE_VALUE;
          const normalMaxY =
            PlaneMaterialCellMatrix.MAX_NORMAL_COORDINATE_VALUE;
          ...
          if (matrix.columns && matrix.columns.length) {
            const cellMatrix = newMaterial.addMaterialCellMatrix(
            ...
            while (index < matrix.columns.length) {
              ...
              if (column)
                this.parsePlaneMaterialCellColumn(column, cellMatrix, index);
  ...
  private parsePlaneMaterialCellColumn(
    column: { repeatMode: string; width: number },
    cellMatrix: PlaneMaterialCellMatrix,
    index: number,
  ): void {
  ...
  private parsePlaneMaterialCells(column: any): PlaneMaterialCell[] {
    ...
    while (index < column.cells.length) {
      ...
      if (cell) {
        ...
        const newCell = new PlaneMaterialCell(
          texture,
          graphics,
          offsetPoints,
          limit,
  ...
  protected getGeometry(
    ...
    const identifier = `${size}_${Math.round(horizontalAngle)}_${Math.round(
      verticalAngle,
    )}`;
    ...
    geometry = new RoomGeometry(
      size,
      new Vector3d(horizontalAngle, verticalAngle),
      new Vector3d(-10, 0, 0),
    );
        ...
        if (planeVisualization && layers && layers.length) {
          ...
          while (layerId < layers.length) {
            ...
            if (layer) {
              ...
              if (layer.materialId)
                material = this.getMaterial(layer.materialId);
              ...
              if (layer.align) {
                if (layer.align === 'bottom')
                  ...
                  align = PlaneVisualizationLayer.ALIGN_TOP;
              }

              planeVisualization.setLayer(
                layerId,
                material,
                color,
                align,
