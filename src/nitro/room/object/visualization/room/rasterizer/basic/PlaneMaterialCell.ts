...
export class PlaneMaterialCell {
  ...
  constructor(
    ...
    assets: IGraphicAsset[] = null,
    offsetPoints: Point[] = null,
    limit: number = 0,
  ) {
  ...
  public render(
    ...
    if (bitmap) {
      if (!this.isStatic) {
        if (this._cachedSprite) {
          if (
            this._cachedSprite.width !== bitmap.width ||
            this._cachedSprite.height !== bitmap.height
          ) {
            this._cachedSprite.destroy();
        ...
        const limitMin = Math.min(
          this._extraItemCount,
          this._extraItemOffsets.length,
        );
        const limitMax = Math.max(
          this._extraItemCount,
          this._extraItemOffsets.length,
        );
        const offsetIndexes = Randomizer.getArray(
          this._extraItemCount,
          limitMax,
        );
