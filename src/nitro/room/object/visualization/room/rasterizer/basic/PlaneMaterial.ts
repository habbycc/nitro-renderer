...
export class PlaneMaterial {
  ...
  public addMaterialCellMatrix(
    ...
    const cellMatrix = new PlaneMaterialCellMatrix(
      totalColumns,
      repeatMode,
      align,
      normalMinX,
  ...
  public getMaterialCellMatrix(normal: IVector3D): PlaneMaterialCellMatrix {
    ...
    if (this._planeMaterialItems && this._planeMaterialItems.length) {
      for (const item of this._planeMaterialItems) {
        ...
        if (
          normal.x >= item.normalMinX &&
          normal.x <= item.normalMaxX &&
          normal.y >= item.normalMinY &&
          normal.y <= item.normalMaxY
        )
          return item;
      }
    }
