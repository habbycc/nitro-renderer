...
export class PlaneMaterialCellMatrix {
  ...
  public static REPEAT_MODE_DEFAULT: number =
  ...
  public render(
    ...
    if (this.isStatic) {
      if (this._cachedBitmapData) {
        if (
          ...
          if (canvas) {
            this.copyCachedBitmapOnCanvas(
              canvas,
              this._cachedBitmapHeight,
              offsetY,
              topAlign,
            );
    ...
    while (columnIndex < this._columns.length) {
      ...
      if (column) {
        const columnBitmapData = column.render(
          height,
          normal,
          offsetX,
          offsetY,
  ...
  private copyCachedBitmapOnCanvas(
    ...
    if (!canvas || !this._cachedBitmapData || canvas === this._cachedBitmapData)
      return;
    ...
    if (this._align == PlaneMaterialCellMatrix.ALIGN_TOP) {
      _local_5 = new Rectangle(
        0,
        0,
        this._cachedBitmapData.width,
        this._cachedBitmapHeight,
      );
    } else {
      _local_5 = new Rectangle(
        0,
        this._cachedBitmapData.height - this._cachedBitmapHeight,
        this._cachedBitmapData.width,
        this._cachedBitmapHeight,
      );
    }
      ...
      if (column) {
        ...
        if (!texture) {
          texture = TextureUtils.generateTexture(
            column,
            new Rectangle(0, 0, column.width, column.height),
          );
  ...
  public getColumns(width: number): PlaneMaterialCellColumn[] {
    if (this._repeatMode === PlaneMaterialCellMatrix.REPEAT_MODE_RANDOM) {
      ...
      while (columnIndex < width) {
        const column =
          this._columns[
            PlaneMaterialCellMatrix.nextRandomColumnIndex(this._columns.length)
          ];
