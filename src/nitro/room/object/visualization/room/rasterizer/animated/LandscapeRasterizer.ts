...
export class LandscapeRasterizer extends PlaneRasterizer {
  ...
  private parseLandscapes(k: any): void {
    ...
    for (const landscapeIndex in k) {
      ...
      for (const visualization of visualizations) {
        ...
        const totalAnimatedLayers =
          (animatedLayers && animatedLayers.length) || 0;
        ...
        if (planeVisualization) {
          ...
          if (totalAnimatedLayers) {
                ...
                if (items && items.length) {
                  for (const item of items) {
                    if (item) {
                      ...
                      const x = this.getCoordinateValue(
                        item.x || '',
                        item.randomX || '',
                      );
                      const y = this.getCoordinateValue(
                        item.y || '',
                        item.randomY || '',
                      );
                      const speedX = item.speedX
                        ? item.speedX /
                          NitroConfiguration.getValue<number>(
            ...
            planeVisualization.setAnimationLayer(
              layerId,
              animationItems,
  ...
  public render(
    ...
    if (!plane)
      plane = this.getPlane(LandscapeRasterizer.DEFAULT) as LandscapePlane;
    ...
    if (!plane.isStatic(scale) && LandscapeRasterizer.UPDATE_INTERVAL > 0) {
      planeBitmapData = new PlaneBitmapData(
        graphic,
        Math.round(timeSinceStartMs / LandscapeRasterizer.UPDATE_INTERVAL) *
          LandscapeRasterizer.UPDATE_INTERVAL +
          LandscapeRasterizer.UPDATE_INTERVAL,
      );
    } else {
