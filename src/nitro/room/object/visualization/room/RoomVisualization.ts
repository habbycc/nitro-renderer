...
export class RoomVisualization
  ...
  implements IPlaneVisualization
{
  public static LAST_VISUALIZATION: RoomVisualization = null;

  public static RENDER_TEXTURE_CACHE: Map<
    RoomVisualization,
    AdvancedMap<any, RenderTexture>
  > = new Map();
  ...
  public static getTextureCache(key: any): RenderTexture {
    const existing = RoomVisualization.RENDER_TEXTURE_CACHE.get(
      RoomVisualization.LAST_VISUALIZATION,
    );
  ...
  public static addTextureCache(key: any, value: RenderTexture): void {
    ...
    let existing = RoomVisualization.RENDER_TEXTURE_CACHE.get(
      RoomVisualization.LAST_VISUALIZATION,
    );
    ...
    if (!existing) {
      ...
      RoomVisualization.RENDER_TEXTURE_CACHE.set(
        RoomVisualization.LAST_VISUALIZATION,
        existing,
      );
    }
  ...
  public dispose(): void {
    ...
    const existingTextureCache =
      RoomVisualization.RENDER_TEXTURE_CACHE.get(this);
  ...
  public update(
    ...
    if (
      time < this._lastUpdateTime + this._updateIntervalTime &&
      !geometryUpdate &&
      !needsUpdate
    )
      return;
      ...
      while (index < this._visiblePlanes.length) {
        ...
        if (sprite && plane && plane.type !== RoomPlane.TYPE_LANDSCAPE) {
          if (this._colorBackgroundOnly) {
            ...
            const _local_16 =
              (((_local_14 >> 8) & 0xff) * this._greenColor) / 0xff;
            const _local_17 =
              (((_local_14 >> 16) & 0xff) * this._blueColor) / 0xff;
  ...
  private updateThickness(k: IRoomObjectModel): boolean {
    ...
    const floorThickness = k.getValue<number>(
      RoomObjectVariable.ROOM_FLOOR_THICKNESS,
    );
    const wallThickness = k.getValue<number>(
      RoomObjectVariable.ROOM_WALL_THICKNESS,
    );

    if (
      !isNaN(floorThickness) &&
      !isNaN(wallThickness) &&
      (floorThickness !== this._floorThickness ||
  ...
  private updateHole(k: IRoomObjectModel): boolean {
    ...
    const holeUpdate = k.getValue<number>(
      RoomObjectVariable.ROOM_FLOOR_HOLE_UPDATE_TIME,
    );
  ...
  private updateMasks(k: IRoomObjectModel): boolean {
    ...
    const planeMask = k.getValue<RoomMapMaskData>(
    ...
    const backgroundColor = k.getValue<number>(
    ...
    const backgroundOnly =
      k.getValue<boolean>(RoomObjectVariable.ROOM_COLORIZE_BG_ONLY) || false;
  ...
  private updatePlaneTexturesAndVisibilities(model: IRoomObjectModel): boolean {
    ...
    const floorVisibility =
      model.getValue<number>(RoomObjectVariable.ROOM_FLOOR_VISIBILITY) === 1;
    const wallVisibility =
      model.getValue<number>(RoomObjectVariable.ROOM_WALL_VISIBILITY) === 1;
    const landscapeVisibility =
      model.getValue<number>(RoomObjectVariable.ROOM_LANDSCAPE_VISIBILITY) ===
      1;
  ...
  protected initializeRoomPlanes(): boolean {
    ...
    const mapData = this.object.model.getValue<RoomMapData>(
      RoomObjectVariable.ROOM_MAP_DATA,
    );
    ...
    let _local_5 = 0;
    let randomSeed = this.object.model.getValue<number>(
    ...
    while (index < this._roomPlaneParser.planeCount) {
      ...
      const secondaryNormals =
        this._roomPlaneParser.getPlaneSecondaryNormals(index);
      ...
      if (location && leftSide && rightSide) {
        ...
        if (plane) {
          ...
          while (_local_19 < this._roomPlaneParser.getPlaneMaskCount(index)) {
            const _local_20 = this._roomPlaneParser.getPlaneMaskLeftSideLoc(
              index,
              _local_19,
            );
            const _local_21 = this._roomPlaneParser.getPlaneMaskRightSideLoc(
              index,
              _local_19,
            );
            const _local_22 = this._roomPlaneParser.getPlaneMaskLeftSideLength(
              index,
              _local_19,
            );
            const _local_23 = this._roomPlaneParser.getPlaneMaskRightSideLength(
              index,
              _local_19,
            );
        ...
        if (_local_12) {
          ...
          if (_local_12.update(k, _arg_3)) {
            if (_local_12.visible) {
              depth =
                _local_12.relativeDepth +
                this.floorRelativeDepth +
                _local_10 / 1000;

              if (_local_12.type !== RoomPlane.TYPE_FLOOR) {
                depth =
                  _local_12.relativeDepth +
                  this.wallRelativeDepth +
                  _local_10 / 1000;

                if (
                  _local_12.leftSide.length < 1 ||
                  _local_12.rightSide.length < 1
                ) {
          ...
          if (
            _local_11.visible !=
            (_local_12.visible && this._typeVisibility[_local_12.type])
          ) {
  ...
  protected updatePlaneMasks(k: RoomMapMaskData): void {
    ...
    for (const mask of this._roomPlaneBitmapMaskParser.masks.values()) {
      ...
      const maskLocation =
        this._roomPlaneBitmapMaskParser.getMaskLocation(mask);
      const maskCategory =
        this._roomPlaneBitmapMaskParser.getMaskCategory(mask);
        ...
        while (i < this._planes.length) {
          ...
          if (
            ...
            if (plane && plane.location && plane.normal) {
              ...
              const _local_15 = Math.abs(
                Vector3d.scalarProjection(_local_14, plane.normal),
              );

              if (_local_15 < 0.01) {
                if (plane.leftSide && plane.rightSide) {
                  const _local_16 = Vector3d.scalarProjection(
                    _local_14,
                    plane.leftSide,
                  );
                  const _local_17 = Vector3d.scalarProjection(
                    _local_14,
                    plane.rightSide,
                  );
  ...
  public getBoundingRectangle(): Rectangle {
    if (!this._boundingRectangle)
      this._boundingRectangle = super.getBoundingRectangle();
