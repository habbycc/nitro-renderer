...
export class SizeData {
  ...
  constructor(layerCount: number, angle: number) {
    this._layerCount =
      layerCount < 0
        ? 0
        : layerCount > SizeData.MAX_LAYERS
        ? SizeData.MAX_LAYERS
        : layerCount;
  ...
  public processDirections(directions: {
  ...
  private setDirectionLayers(
    ...
    for (const key in layers) {
      ...
      if (layer.ink !== undefined)
        directionData.setLayerInk(
          layerId,
          SpriteUtilities.inkToBlendMode(layer.ink),
        );
