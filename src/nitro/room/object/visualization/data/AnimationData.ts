...
export class AnimationData {
  ...
  public initialize(k: IAssetVisualAnimation): boolean {
    ...
    if (k.layers) {
      for (const key in k.layers) {
        ...
        const frameRepeat =
          layer.frameRepeat !== undefined ? layer.frameRepeat : 1;
        const isRandom =
          layer.random !== undefined && layer.random !== 0 ? true : false;

        if (
          !this.addLayer(animationId, loopCount, frameRepeat, isRandom, layer)
        )
          return false;
      }
    }
  ...
  private addLayer(
    ...
    if (layer.frameSequences) {
      for (const key in layer.frameSequences) {
        ...
        const loopCount =
          animationSequence.loopCount !== undefined
            ? animationSequence.loopCount
            : 1;
        const isSequenceRandom =
          animationSequence.random !== undefined &&
          animationSequence.random !== 0
            ? true
            : false;
  ...
  private readDirectionalOffsets(
    frame: IAssetVisualAnimationSequenceFrame,
  ...
  public getFrameFromSequence(
    ...
    return layer.getFrameFromSequence(
      direction,
      sequenceId,
      offset,
      frameCount,
