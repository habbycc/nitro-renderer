...
export class AnimationLayerData {
  ...
  public getFrame(direction: number, frameCount: number): AnimationFrame {
    ...
    if (!this._isRandom) {
      ...
      if (
        (this._loopCount > 0 && count >= this._loopCount) ||
        (this._loopCount <= 0 && this._frameCount === 1)
      ) {
      ...
      return this.getFrameFromSpecificSequence(
        direction,
        sequence,
        sequenceId,
  ...
  private getFrameFromSpecificSequence(
    ...
    if (!this._isRandom && !sequence.isRandom) {
      if (
        sequenceId === this._frameSequences.length - 1 &&
        offset === sequence.frameCount - 1
      )
        isLastFrame = true;
    }
