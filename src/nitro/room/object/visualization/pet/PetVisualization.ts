...
export class PetVisualization extends FurnitureAnimatedVisualization {
  ...
  private static PET_EXPERIENCE_BUBBLE: string =
    'avatar_addition_pet_experience_bubble';
  ...
  constructor() {
    ...
    while (
      this._animationStates.length < PetVisualization.ANIMATION_INDEX_COUNT
    )
  ...
  public initialize(data: IObjectVisualizationData): boolean {
    ...
    const texture = this.getPetAdditionAsset(
      PetVisualization.PET_EXPERIENCE_BUBBLE,
    );
    ...
    if (this._experienceTimestamp) {
      ...
      if (difference < PetVisualization.EXPERIENCE_BUBBLE_VISIBLE_IN_MS) {
        this._experienceData.alpha =
          Math.sin(
            (difference / PetVisualization.EXPERIENCE_BUBBLE_VISIBLE_IN_MS) *
              Math.PI,
          ) * 0xff;
      } else {
        this._experienceTimestamp = 0;
  ...
  protected updateModel(scale: number): boolean {
    ...
    let alphaMultiplier =
      model.getValue<number>(RoomObjectVariable.FURNITURE_ALPHA_MULTIPLIER) ||
      null;
    ...
    this._isSleeping =
      model.getValue<number>(RoomObjectVariable.FIGURE_SLEEP) > 0;
    ...
    this._experience = model.getValue<number>(
      RoomObjectVariable.FIGURE_GAINED_EXPERIENCE,
    );
    this._experienceTimestamp = model.getValue<number>(
      RoomObjectVariable.FIGURE_EXPERIENCE_TIMESTAMP,
    );

    const customPaletteIndex = model.getValue<number>(
      RoomObjectVariable.PET_PALETTE_INDEX,
    );
    const customLayerIds = model.getValue<number[]>(
      RoomObjectVariable.PET_CUSTOM_LAYER_IDS,
    );
    const customPartIds = model.getValue<number[]>(
      RoomObjectVariable.PET_CUSTOM_PARTS_IDS,
    );
    const customPaletteIds = model.getValue<number[]>(
  ...
  protected setPostureAndGesture(posture: string, gesture: string): void {
    ...
    if (posture !== this._posture) {
      ...
      this.setAnimationForIndex(
        ...
        this._data.postureToAnimation(this._scale, posture),
      );
    }
    ...
    if (gesture !== this._gesture) {
      ...
      this.setAnimationForIndex(
        ...
        this._data.gestureToAnimation(this._scale, gesture),
      );
    }
  }
        ...
        if (!stateData.animationOver) {
          ...
          if (!stateData.animationOver) {
            ...
            if (
              ...
              this.setAnimationForIndex(
                index,
                stateData.animationAfterTransitionId,
              );
  ...
  protected getSpriteAssetName(scale: number, layerId: number): string {
    ...
    if (layerId < totalSprites - PetVisualization.ADDITIONAL_SPRITE_COUNT) {
      ...
      if (
        layerId <
        totalSprites - (1 + PetVisualization.ADDITIONAL_SPRITE_COUNT)
      ) {
        if (layerId >= FurnitureVisualizationData.LAYER_LETTERS.length)
          return null;
  ...
  protected getLayerColor(
    ...
    if (layerId < this.totalSprites - PetVisualization.ADDITIONAL_SPRITE_COUNT)
      return this._color;

    return 0xffffff;
  }

  protected getLayerXOffset(
    scale: number,
    direction: number,
    layerId: number,
  ): number {
  ...
  protected getLayerZOffset(
    ...
    return this._data.getLayerZOffset(
      scale,
      this.getDirection(scale, layerId),
      layerId,
    );
  ...
  private isHeadSprite(layerId: number): boolean {
    if (this._headSprites[layerId] === undefined) {
      const isHead =
        this._data.getLayerTag(
          this._scale,
          DirectionData.USE_DEFAULT_DIRECTION,
          layerId,
  ...
  private isNonHeadSprite(layerId: number): boolean {
    if (this._nonHeadSprites[layerId] === undefined) {
      if (
        layerId <
        this.totalSprites - (1 + PetVisualization.ADDITIONAL_SPRITE_COUNT)
      ) {
        const tag = this._data.getLayerTag(
          this._scale,
          DirectionData.USE_DEFAULT_DIRECTION,
          layerId,
        );
        ...
        if (
          tag &&
          tag.length > 0 &&
          tag !== PetVisualization.HEAD &&
          tag !== PetVisualization.HAIR
        ) {
  ...
  protected getPostureForAsset(scale: number, name: string): string {
    ...
    if (length < parts.length) {
      ...
      posture = this._data.animationToPosture(
        scale,
        parseInt(part) / 100,
        false,
      );
      ...
      if (!posture)
        posture = this._data.getGestureForAnimationId(
          scale,
          parseInt(part) / 100,
        );
    }
