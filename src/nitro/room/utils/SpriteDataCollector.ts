...
export class SpriteDataCollector {
  ...
  private static addMannequinSprites(
    ...
    for (const data of k) {
      ...
      if (
        data.type === 'boutique_mannequin1' &&
        data.name.indexOf('mannequin_') === 0
      ) {
        const roomObject = _arg_2.getRoomObject(
          _arg_2.activeRoomId,
          data.objectId,
          ...
          if (spriteList) {
            for (const sprite of spriteList) {
              sprite.x =
                ...
                (data.x +
                  data.width / 2 +
                  SpriteDataCollector.MANNEQUIN_MAGIC_X_OFFSET);
              sprite.y =
                sprite.y +
                (data.y +
                  data.height +
                  SpriteDataCollector.MANNEQUIN_MAGIC_Y_OFFSET);
  ...
  public getFurniData(
    ...
    const _local_7 = _arg_3.getRoomObjects(
      _arg_3.activeRoomId,
    ...
    for (const _local_8 of _local_7) {
      if (_local_8.id !== _arg_4) {
        const _local_11 = (
          _local_8.visualization as IRoomObjectSpriteVisualization
        ).getSpriteList();
        ...
        if (_local_11) {
          ...
          for (const _local_14 of _local_6) {
            if (_local_14.name === 'avatar_' + _local_8.id) {
              ...
              _local_13 =
                _local_14.y + _local_14.height - _arg_2.geometry.scale / 4;

              break;
            }
          }

          const _local_15 = _arg_3.getRoomObjectScreenLocation(
            _arg_3.activeRoomId,
            _local_8.id,
            RoomObjectCategory.UNIT,
            _arg_2.id,
          ...
          if (_local_15) {
            ...
            for (const _local_16 of _local_11) {
              ...
              if (
                _local_16.name.indexOf('h_std_fx29_') === 0 ||
                _local_16.name.indexOf('h_std_fx185_') === 0
              ) {
                _local_16.y =
                  _local_16.y +
                  SpriteDataCollector.AVATAR_WATER_EFFECT_MAGIC_Y_OFFSET;
              }
  ...
  private getSpriteDataObject(
    ...
    if (_local_6.indexOf('http') === 0) {
      ...
      if (
        this.externalImageCount > SpriteDataCollector.MAX_EXTERNAL_IMAGE_COUNT
      )
        _local_5.name = 'box';
    }
  ...
  private makeBackgroundPlane(
    ...
    const _local_8 = SpriteDataCollector.sortQuadPoints(
      _local_4,
      _local_5,
      _local_6,
      _local_7,
    ...
    _local_9 =
      _local_9 + (this.spriteCount * 1.776104 + _arg_3.length * 2.31743);
  ...
  public getRoomPlanes(
    ...
    const roomObject = _arg_3.getRoomObject(
      _arg_3.activeRoomId,
      RoomEngine.ROOM_OBJECT_ID,
      RoomObjectCategory.ROOM,
    );
    const visualization =
      roomObject.visualization as unknown as IPlaneVisualization;

    if (visualization) {
      const _local_8 = _arg_2.geometry;
      const _local_9 = this.sortRoomPlanes(
      ...
      for (const _local_11 of _local_9) {
        ...
        const _local_17 = _local_8.getScreenPoint(
          Vector3d.sum(_local_12.location, _local_12.rightSide),
        );
        const _local_18 = _local_8.getScreenPoint(
          Vector3d.sum(_local_14, _local_12.rightSide),
        );
        ...
        if (Math.abs(_local_19) === 4 || Math.abs(_local_20) === 4) {
          ...
          const _local_22 = SpriteDataCollector.sortQuadPoints(
            _local_15,
            _local_16,
            _local_17,
            _local_18,
