...
export class RoomContentLoader
  implements IFurnitureDataListener, IRoomContentLoader
{
  ...
  public initialize(events: IEventDispatcher): void {
    ...
    for (const [index, name] of NitroConfiguration.getValue<string[]>(
      'pet.types',
    ).entries())
  ...
  private setFurnitureData(): void {
    ...
    this._stateEvents.dispatchEvent(
      new NitroEvent(RoomContentLoader.LOADER_READY),
    );
  }
  ...
  private processFurnitureData(furnitureData: IFurnitureData[]): void {
    ...
    for (const furniture of furnitureData) {
      ...
      if (furniture.hasIndexedColor)
        className = className + '*' + furniture.colorIndex;

      const revision = furniture.revision;
      const adUrl = furniture.adUrl;

      if (adUrl && adUrl.length > 0)
        this._objectTypeAdUrls.set(className, adUrl);
  ...
  public getFurnitureWallNameForTypeId(
    typeId: number,
    extra: string = null,
  ): string {
  ...
  public createGifCollection(
    ...
    const collection = new GraphicAssetGifCollection(
      collectionName,
      textures,
      durations,
    );
  ...
  private createCollection(
    ...
    if (petIndex !== undefined) {
      ...
      for (const key of keys) {
        ...
        const tag =
          paletteData.colorTag !== undefined ? paletteData.colorTag : -1;
        const master =
          paletteData.master !== undefined ? paletteData.master : false;
        const layerTags =
          paletteData.tags !== undefined ? paletteData.tags : [];
  ...
  public getCategoryForType(type: string): number {
    ...
    if (type === RoomObjectUserType.RENTABLE_BOT)
      return RoomObjectCategory.UNIT;

    if (
      type === RoomContentLoader.TILE_CURSOR ||
      type === RoomContentLoader.SELECTION_ARROW
    )
  ...
  public downloadImage(
    ...
    if (assetUrls && assetUrls.length) {
      ...
      image.onload = () => {
        ...
        this._iconListener.onRoomContentLoaded(
          id,
          [type, param].join('_'),
          true,
        );
  ...
  public downloadAsset(type: string, events: IEventDispatcher): void {
    ...
    if (
      this._pendingContentTypes.indexOf(type) >= 0 ||
      this.getOrRemoveEventDispatcher(type)
    )
      return;
    ...
    const onDownloaded = (status: boolean, url: string) => {
      if (!status) {
        ...
        events.dispatchEvent(
          new RoomContentLoadedEvent(RoomContentLoadedEvent.RCLE_FAILURE, type),
        );
      ...
      if (!remaining) {
        ...
        events.dispatchEvent(
          new RoomContentLoadedEvent(RoomContentLoadedEvent.RCLE_SUCCESS, type),
        );
  ...
  private processAsset(
    ...
    data: IAssetData,
    onDownloaded: (status: boolean) => void,
  ): void {
    ...
    if (
      !baseTexture ||
      !spritesheetData ||
      !Object.keys(spritesheetData).length
    ) {
  ...
  public getAssetUrls(
    ...
    switch (type) {
      case RoomContentLoader.PLACE_HOLDER:
        return [
          this.getAssetUrlWithGenericBase(RoomContentLoader.PLACE_HOLDER),
        ];
      case RoomContentLoader.PLACE_HOLDER_WALL:
        return [
          this.getAssetUrlWithGenericBase(RoomContentLoader.PLACE_HOLDER_WALL),
        ];
      case RoomContentLoader.PLACE_HOLDER_PET:
        return [
          this.getAssetUrlWithGenericBase(RoomContentLoader.PLACE_HOLDER_PET),
        ];
      ...
      case RoomContentLoader.SELECTION_ARROW:
        return [
          this.getAssetUrlWithGenericBase(RoomContentLoader.SELECTION_ARROW),
        ];
      default: {
        const category = this.getCategoryForType(type);
        ...
        if (
          ...
          let assetUrl = icon
            ? this.getAssetUrlWithFurniIconBase(name)
            : this.getAssetUrlWithFurniBase(type);

          if (icon) {
            const active =
              param &&
              param !== '' &&
              this._activeObjectTypeIds.has(name + '*' + param);
  ...
  private getAssetUrlWithGenericBase(assetName: string): string {
    return NitroConfiguration.getValue<string>('generic.asset.url').replace(
      /%libname%/gi,
      assetName,
    );
  }
  ...
  public getAssetUrlWithFurniBase(assetName: string): string {
    return NitroConfiguration.getValue<string>('furni.asset.url').replace(
      /%libname%/gi,
      assetName,
    );
  }
  ...
  public getAssetUrlWithFurniIconBase(assetName: string): string {
    return NitroConfiguration.getValue<string>('furni.asset.icon.url').replace(
      /%libname%/gi,
      assetName,
    );
  }
  ...
  public getAssetUrlWithPetBase(assetName: string): string {
    return NitroConfiguration.getValue<string>('pet.asset.url').replace(
      /%libname%/gi,
      assetName,
    );
  }
