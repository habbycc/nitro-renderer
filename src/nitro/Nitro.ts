...
settings.SCALE_MODE = !(window.devicePixelRatio % 1)
  ? SCALE_MODES.NEAREST
  : SCALE_MODES.LINEAR;
...
export class Nitro implements INitro {
  ...
  constructor(core: INitroCore, options?: IApplicationOptions) {
    ...
    this._roomSessionManager = new RoomSessionManager(
      this._communication,
      this._roomEngine,
    );
    this._roomManager = new RoomManager(
      this._roomEngine,
      this._roomEngine.visualizationFactory,
      this._roomEngine.logicFactory,
    );
    ...
    this._core.configuration.events.addEventListener(
      ConfigurationEvent.LOADED,
      this.onConfigurationLoadedEvent.bind(this),
    );
  ...
  private onConfigurationLoadedEvent(event: ConfigurationEvent): void {
    const animationFPS = NitroConfiguration.getValue<number>(
      'system.animation.fps',
      24,
    );
    const limitsFPS = NitroConfiguration.getValue<boolean>(
      'system.limits.fps',
      false,
    );
  ...
  public getLocalizationWithParameter(
    key: string,
    parameter: string,
    replacement: string,
  ): string {
    return this._localization.getValueWithParameter(
      key,
      parameter,
      replacement,
    );
  ...
  public getLocalizationWithParameters(
    key: string,
    parameters: string[],
    replacements: string[],
  ): string {
    return this._localization.getValueWithParameters(
      key,
      parameters,
      replacements,
    );
