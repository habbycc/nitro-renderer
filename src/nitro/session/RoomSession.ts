...
export class RoomSession extends Disposable implements IRoomSession {
  ...
  public setControllerLevel(level: number): void {
    if (
      level >= RoomControllerLevel.NONE &&
      level <= RoomControllerLevel.MODERATOR
    ) {
  ...
  public start(): boolean {
    if (this._state !== RoomSessionEvent.CREATED || !this._connection)
      return false;
  ...
  public sendWhisperMessage(
    recipientName: string,
    text: string,
    styleId: number,
  ): void {
    this._connection.send(
      new RoomUnitChatWhisperComposer(recipientName, text, styleId),
    );
  }
  ...
  public sendMuteMessage(userId: number, minutes: number): void {
    this._connection.send(
      new RoomMuteUserComposer(userId, minutes, this._roomId),
    );
  }
  ...
  public updateMoodlightData(
    ...
    this.connection.send(
      new MoodlightSettingsSaveComposer(
        id,
        effectId,
        colorString,
        brightness,
