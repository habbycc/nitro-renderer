...
export class BadgeImageManager implements IDisposable {
  ...
  public getBadgeImage(
    badgeName: string,
    type: string = BadgeImageManager.NORMAL_BADGE,
    load: boolean = true,
  ): Texture<Resource> {
  ...
  public getBadgeInfo(k: string): BadgeInfo {
    ...
    return badge
      ? new BadgeInfo(badge, false)
      : new BadgeInfo(this.getBadgePlaceholder(), true);
  }

  public loadBadgeImage(
    badgeName: string,
    type: string = BadgeImageManager.NORMAL_BADGE,
  ): string {
    ...
    if (type === BadgeImageManager.NORMAL_BADGE) {
      ...
      this._assets.downloadAsset(url, (flag: boolean) => {
        if (flag) {
          ...
          if (texture && this._sessionDataManager)
            this._sessionDataManager.events.dispatchEvent(
              new BadgeImageReadyEvent(badgeName, texture.clone()),
            );
        }
      });
  ...
  private getBadgePlaceholder(): Texture<Resource> {
    const url =
      NitroConfiguration.getValue<string>('images.url') + '/loading_icon.png';
    const existing = this._assets.getTexture(url);
  ...
  public getBadgeUrl(
    ...
    switch (type) {
      case BadgeImageManager.NORMAL_BADGE:
        url = NitroConfiguration.getValue<string>('badge.asset.url').replace(
          '%badgename%',
          badge,
        );
        break;
  ...
  private loadGroupBadge(badgeCode: string): void {
    ...
    for (const partMatch of partMatches) {
      ...
      const partColor = parseInt(
        partCode.slice(shortMethod ? 3 : 4, shortMethod ? 5 : 6),
      );
      const partPosition =
        partCode.length < 6
          ? 0
          : parseInt(partCode.slice(shortMethod ? 5 : 6, shortMethod ? 6 : 7)); // sometimes position is ommitted
      const part = new GroupBadgePart(
        partType,
        partId,
        partColor,
        partPosition,
  ...
  private renderGroupBadge(groupBadge: GroupBadge): void {
    ...
    for (const part of groupBadge.parts) {
      ...
      const partNames =
        part.type === 'b'
          ? this._groupBases.get(part.key)
          : this._groupSymbols.get(part.key);
    ...
    if (this._sessionDataManager)
      this._sessionDataManager.events.dispatchEvent(
        new BadgeImageReadyEvent(groupBadge.code, texture),
      );
  }
  ...
  private onGroupBadgePartsEvent(event: GroupBadgePartsEvent): void {
    ...
    for (const badgeCode of this._groupBadgesQueue.keys())
