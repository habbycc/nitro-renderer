...
export class GroupBadgePart {
  ...
  public static getCode(
    ...
    return (
      (type === GroupBadgePart.BASE
        ? type
        : key >= 100
        ? GroupBadgePart.SYMBOL_ALT
        : GroupBadgePart.SYMBOL) +
      (key < 10 ? '0' : '') +
  ...
  public calculatePosition(asset: NitroTexture): NitroPoint {
    ...
    let x: number =
      GroupBadgePart.CELL_WIDTH * gridPos.x +
      GroupBadgePart.CELL_WIDTH / 2 -
      asset.width / 2;
    let y: number =
      GroupBadgePart.CELL_HEIGHT * gridPos.y +
      GroupBadgePart.CELL_HEIGHT / 2 -
      asset.height / 2;

    if (x < 0) x = 0;

    if (x + asset.width > GroupBadgePart.IMAGE_WIDTH)
      x = GroupBadgePart.IMAGE_WIDTH - asset.width;

    if (y < 0) y = 0;

    if (y + asset.height > GroupBadgePart.IMAGE_HEIGHT)
      y = GroupBadgePart.IMAGE_HEIGHT - asset.height;
