  ...
  implements ISessionDataManager
{
  ...
  protected onInit(): void {
    ...
    this._communication.registerMessageEvent(
      new FigureUpdateEvent(this.onUserFigureEvent.bind(this)),
    );
    this._communication.registerMessageEvent(
      new UserInfoEvent(this.onUserInfoEvent.bind(this)),
    );
    this._communication.registerMessageEvent(
      new UserPermissionsEvent(this.onUserPermissionsEvent.bind(this)),
    );
    this._communication.registerMessageEvent(
      new AvailabilityStatusMessageEvent(
        this.onAvailabilityStatusMessageEvent.bind(this),
      ),
    );
    this._communication.registerMessageEvent(
      new ChangeUserNameResultMessageEvent(
        this.onChangeNameUpdateEvent.bind(this),
      ),
    );
    this._communication.registerMessageEvent(
      new UserNameChangeMessageEvent(
        this.onUserNameChangeMessageEvent.bind(this),
      ),
    );
    this._communication.registerMessageEvent(
      new RoomReadyMessageEvent(this.onRoomModelNameEvent.bind(this)),
    );
    this._communication.registerMessageEvent(
      new InClientLinkEvent(this.onInClientLinkEvent.bind(this)),
    );
    this._communication.registerMessageEvent(
      new MysteryBoxKeysEvent(this.onMysteryBoxKeysEvent.bind(this)),
    );
    this._communication.registerMessageEvent(
      new NoobnessLevelMessageEvent(
        this.onNoobnessLevelMessageEvent.bind(this),
      ),
    );

    Nitro.instance.events.addEventListener(
      NitroSettingsEvent.SETTINGS_UPDATED,
      this.onNitroSettingsEvent,
    );
  }
  ...
  private loadFurnitureData(): void {
    ...
    this._furnitureData = new FurnitureDataLoader(
      this._floorItems,
      this._wallItems,
      Nitro.instance.localization,
    );

    this._furnitureData.addEventListener(
      FurnitureDataLoader.FURNITURE_DATA_READY,
      this.onFurnitureDataReadyEvent,
    );

    this._furnitureData.loadFurnitureData(
      NitroConfiguration.getValue<string>('furnidata.url'),
    );
  }
  ...
  private loadProductData(): void {
    ...
    this._productData.addEventListener(
      ProductDataLoader.PDP_PRODUCT_DATA_READY,
      this.onProductDataReadyEvent,
    );

    this._productData.loadProductData(
      NitroConfiguration.getValue<string>('productdata.url'),
    );
  }
  ...
  public hasProductData(listener: IProductDataListener): boolean {
    ...
    if (listener && this._pendingProductListeners.indexOf(listener) === -1)
      this._pendingProductListeners.push(listener);

    return false;
  }

  public getAllFurnitureData(
  ...
  private onAvailabilityStatusMessageEvent(
    event: AvailabilityStatusMessageEvent,
  ): void {
    if (!event || !event.connection) return;
  ...
  private onChangeNameUpdateEvent(
    event: ChangeUserNameResultMessageEvent,
  ): void {
    if (!event || !event.connection) return;
  ...
  private onUserNameChangeMessageEvent(
    event: UserNameChangeMessageEvent,
  ): void {
    if (!event || !event.connection) return;
  ...
  private onFurnitureDataReadyEvent(event: INitroEvent): void {
    this._furnitureData.removeEventListener(
      FurnitureDataLoader.FURNITURE_DATA_READY,
      this.onFurnitureDataReadyEvent,
    );
    ...
    if (!this._furnitureListenersNotified) {
      ...
      if (
        this._pendingFurnitureListeners &&
        this._pendingFurnitureListeners.length
      ) {
        for (const listener of this._pendingFurnitureListeners)
  ...
  private onProductDataReadyEvent(event: INitroEvent): void {
    this._productData.removeEventListener(
      ProductDataLoader.PDP_PRODUCT_DATA_READY,
      this.onProductDataReadyEvent,
    );
    ...
    for (const listener of this._pendingProductListeners)
      listener && listener.loadProductData();
  ...
  private onMysteryBoxKeysEvent(event: MysteryBoxKeysEvent): void {
    ...
    this.events.dispatchEvent(
      new MysteryBoxKeysUpdateEvent(parser.boxColor, parser.keyColor),
    );
  }
  ...
  public getGroupBadgeUrl(name: string): string {
    return this._badgeImageManager.getBadgeUrl(
      name,
      BadgeImageManager.GROUP_BADGE,
    );
  }
  ...
  public getGroupBadgeImage(name: string): Texture<Resource> {
    return this._badgeImageManager.getBadgeImage(
      name,
      BadgeImageManager.GROUP_BADGE,
    );
  }
  ...
  public loadGroupBadgeImage(name: string): string {
    return this._badgeImageManager.loadBadgeImage(
      name,
      BadgeImageManager.GROUP_BADGE,
    );
  }
  ...
  public isUserIgnored(name: string): boolean {
    return (
      this._ignoredUsersManager && this._ignoredUsersManager.isIgnored(name)
    );
  }

  public getGroupBadge(groupId: number): string {
    return (
      this._groupInformationManager &&
      this._groupInformationManager.getGroupBadge(groupId)
    );
  }
