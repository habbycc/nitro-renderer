...
export class WordQuizHandler extends BaseHandler {
  constructor(connection: IConnection, listener: IRoomHandlerListener) {
    ...
    connection.addMessageEvent(
      new QuestionEvent(this.onQuestionEvent.bind(this)),
    );
    connection.addMessageEvent(
      new QuestionAnsweredEvent(this.onQuestionAnsweredEvent.bind(this)),
    );
    connection.addMessageEvent(
      new QuestionFinishedEvent(this.onQuestionFinishedEvent.bind(this)),
    );
  }
  ...
  private onQuestionEvent(event: QuestionEvent): void {
    ...
    const quizEvent = new RoomSessionWordQuizEvent(
      RoomSessionWordQuizEvent.QUESTION,
      session,
      parser.pollId,
    );
  ...
  private onQuestionAnsweredEvent(event: QuestionAnsweredEvent): void {
    ...
    const quizEvent = new RoomSessionWordQuizEvent(
      RoomSessionWordQuizEvent.ANSWERED,
      session,
      parser.userId,
    );
  ...
  private onQuestionFinishedEvent(event: QuestionFinishedEvent): void {
    ...
    const quizEvent = new RoomSessionWordQuizEvent(
      RoomSessionWordQuizEvent.FINISHED,
      session,
    );
    quizEvent.questionId = parser.questionId;
    quizEvent.answerCounts = parser.answerCounts;
