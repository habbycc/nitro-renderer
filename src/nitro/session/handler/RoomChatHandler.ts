...
export class RoomChatHandler extends BaseHandler {
  constructor(connection: IConnection, listener: IRoomHandlerListener) {
    ...
    connection.addMessageEvent(
      new RoomUnitHandItemReceivedEvent(
        this.onRoomUnitHandItemReceivedEvent.bind(this),
      ),
    );
    connection.addMessageEvent(
      new RespectReceivedEvent(this.onRespectReceivedEvent.bind(this)),
    );
    connection.addMessageEvent(
      new PetRespectNoficationEvent(
        this.onPetRespectNoficationEvent.bind(this),
      ),
    );
    connection.addMessageEvent(
      new PetSupplementedNotificationEvent(
        this.onPetSupplementedNotificationEvent.bind(this),
      ),
    );
    connection.addMessageEvent(
      new FloodControlEvent(this.onFloodControlEvent.bind(this)),
    );
    connection.addMessageEvent(
      new RemainingMuteEvent(this.onRemainingMuteEvent.bind(this)),
    );
  }
  ...
  private onRoomUnitChatEvent(event: RoomUnitChatEvent): void {
    ...
    if (event instanceof RoomUnitChatShoutEvent)
      chatType = RoomSessionChatEvent.CHAT_TYPE_SHOUT;
    else if (event instanceof RoomUnitChatWhisperEvent)
  ...
  private onRoomUnitHandItemReceivedEvent(
    event: RoomUnitHandItemReceivedEvent,
  ): void {
    if (!this.listener) return;
  ...
  private onPetSupplementedNotificationEvent(
    event: PetSupplementedNotificationEvent,
  ): void {
    if (!this.listener) return;
