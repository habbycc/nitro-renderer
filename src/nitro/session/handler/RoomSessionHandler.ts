...
export class RoomSessionHandler extends BaseHandler {
  ...
  constructor(connection: IConnection, listener: IRoomHandlerListener) {
    ...
    connection.addMessageEvent(
      new RoomDoorbellAcceptedEvent(
        this.onRoomDoorbellAcceptedEvent.bind(this),
      ),
    );
    connection.addMessageEvent(
      new FlatAccessDeniedMessageEvent(
        this.onRoomDoorbellRejectedEvent.bind(this),
      ),
    );
    connection.addMessageEvent(
      new YouAreSpectatorMessageEvent(
        this.onYouAreSpectatorMessageEvent.bind(this),
      ),
    );
  }
  ...
  private onDesktopViewEvent(event: DesktopViewEvent): void {
    ...
    if (this.listener)
      this.listener.sessionUpdate(
        this.roomId,
        RoomSessionHandler.RS_DISCONNECTED,
      );
  }
  ...
  private onRoomDoorbellRejectedEvent(
    event: FlatAccessDeniedMessageEvent,
  ): void {
    if (!(event instanceof FlatAccessDeniedMessageEvent) || !this.listener)
      return;
  ...
  private onYouAreSpectatorMessageEvent(
    event: YouAreSpectatorMessageEvent,
  ): void {
    if (this.listener) {
      const session = this.listener.getSession(this.roomId);
      ...
      this.listener.events.dispatchEvent(
        new RoomSessionSpectatorModeEvent(
          RoomSessionSpectatorModeEvent.SPECTATOR_MODE,
          session,
        ),
      );
    }
