...
export class RoomUsersHandler extends BaseHandler {
  constructor(connection: IConnection, listener: IRoomHandlerListener) {
    ...
    connection.addMessageEvent(
      new DoorbellMessageEvent(this.onRoomDoorbellEvent.bind(this)),
    );
    connection.addMessageEvent(
    ...
    connection.addMessageEvent(
      new PetInfoEvent(this.onPetInfoEvent.bind(this)),
    );
    connection.addMessageEvent(
      new PetStatusUpdateEvent(this.onPetStatusUpdateEvent.bind(this)),
    );
    connection.addMessageEvent(
      new PetFigureUpdateEvent(this.onPetFigureUpdateEvent.bind(this)),
    );
    connection.addMessageEvent(
      new PetPlacingErrorEvent(this.onPetPlacingError.bind(this)),
    );
    connection.addMessageEvent(new BotErrorEvent(this.onBotError.bind(this)));
    connection.addMessageEvent(
      new FavoriteMembershipUpdateMessageEvent(
        this.onFavoriteMembershipUpdateMessageEvent.bind(this),
      ),
    );
  }
  ...
  private onRoomUnitEvent(event: RoomUnitEvent): void {
    ...
    this.listener.events.dispatchEvent(
      new RoomSessionUserDataUpdateEvent(session, usersToAdd),
    );
  }
  ...
  private onRoomUnitInfoEvent(event: RoomUnitInfoEvent): void {
    ...
    session.userDataManager.updateFigure(
      parser.unitId,
      parser.figure,
      parser.gender,
      false,
    ...
    session.userDataManager.updateAchievementScore(
      parser.unitId,
  ...
  private onRoomUnitDanceEvent(event: RoomUnitDanceEvent): void {
    ...
    this.listener.events.dispatchEvent(
      new RoomSessionDanceEvent(session, parser.unitId, parser.danceId),
    );
  }
  ...
  private onPetInfoEvent(event: PetInfoEvent): void {
    ...
    this.listener.events.dispatchEvent(
      new RoomSessionPetInfoUpdateEvent(session, petData),
    );
  }
  ...
  private onPetPlacingError(event: PetPlacingErrorEvent): void {
    ...
    this.listener.events.dispatchEvent(
      new RoomSessionErrorMessageEvent(type, session),
    );
  }
  ...
  private onFavoriteMembershipUpdateMessageEvent(
    event: FavoriteMembershipUpdateMessageEvent,
  ): void {
    if (!this.listener) return;
    ...
    this.listener.events.dispatchEvent(
      new RoomSessionFavoriteGroupUpdateEvent(
        session,
        parser.roomIndex,
        parser.groupId,
        parser.status,
