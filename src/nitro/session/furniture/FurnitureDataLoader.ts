...
export class FurnitureDataLoader extends EventDispatcher {
  ...
  private onFurnitureDataLoaded(data: { [index: string]: any }): void {
    ...
    if (
      typeof data.roomitemtypes == 'undefined' ||
      typeof data.wallitemtypes == 'undefined'
    )
      NitroLogger.warn(
        'Could not find `roomitemtypes` or `wallitemtypes` in furnidata.',
      );
    ...
    this.dispatchEvent(
      new NitroEvent(FurnitureDataLoader.FURNITURE_DATA_READY),
    );
  }
  ...
  private onFurnitureDataError(error: Error): void {
    ...
    this.dispatchEvent(
      new NitroEvent(FurnitureDataLoader.FURNITURE_DATA_ERROR),
    );
  }
  ...
  private updateLocalizations(furniture: FurnitureData): void {
    ...
    switch (furniture.type) {
      case FurnitureType.FLOOR:
        this._localization.setValue(
          'roomItem.name.' + furniture.id,
          furniture.name,
        );
        this._localization.setValue(
          'roomItem.desc.' + furniture.id,
          furniture.description,
        );
        return;
      case FurnitureType.WALL:
        this._localization.setValue(
          'wallItem.name.' + furniture.id,
          furniture.name,
        );
        this._localization.setValue(
          'wallItem.desc.' + furniture.id,
          furniture.description,
        );
        return;
    }
