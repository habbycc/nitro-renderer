...
export class ProductDataLoader extends EventDispatcher {
  ...
  private onProductDataLoadedEvent(data: { [index: string]: any }): void {
    ...
    this.dispatchEvent(
      new NitroEvent(ProductDataLoader.PDP_PRODUCT_DATA_READY),
    );
  }
  ...
  private onProductDataError(error: Error): void {
    ...
    this.dispatchEvent(
      new NitroEvent(ProductDataLoader.PDP_PRODUCT_DATA_FAILED),
    );
  }
  ...
  private parseProducts(data: { [index: string]: any }): void {
    ...
    for (const product of data.product)
      product &&
        this._products.set(
          product.code,
          new ProductData(product.code, product.name, product.description),
        );
  }
}
