...
export class Motions {
  ...
  public static runMotion(k: Motion): Motion {
    if (
      Motions._RUNNING_MOTIONS.indexOf(k) === -1 &&
      Motions._QUEUED_MOTIONS.indexOf(k) === -1
    ) {
  ...
  public static getMotionByTagAndTarget(
    k: string,
    _arg_2: HTMLElement,
  ): Motion {
  ...
  private static onTick(): void {
    ...
    while ((_local_3 = Motions._QUEUED_MOTIONS.pop()))
      Motions._RUNNING_MOTIONS.push(_local_3);

    // eslint-disable-next-line no-cond-assign
    while ((_local_3 = Motions._REMOVED_MOTIONS.pop())) {
      Motions._RUNNING_MOTIONS.splice(
        Motions._RUNNING_MOTIONS.indexOf(_local_3),
        1,
      );
