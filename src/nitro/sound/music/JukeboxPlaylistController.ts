...
export class JukeboxPlaylistController implements IPlaylistController {
  ...
  constructor() {
    ...
    this._messageEvents = [
      ...
      new JukeboxSongDisksMessageEvent(
        this.onJukeboxSongDisksMessageEvent.bind(this),
      ),
      new JukeboxPlayListFullMessageEvent(
        this.onJukeboxPlayListFullMessageEvent.bind(this),
      ),
    ];
  ...
  private onNowPlayingMessageEvent(event: NowPlayingMessageEvent): void {
    ...
    console.log(
      'Received Now Playing message with: ' +
        parser.currentSongId +
        ', ' +
        parser.nextSongId +
        ', ' +
        parser.syncCount,
    ...
    if (parser.currentSongId >= 0) {
      Nitro.instance.soundManager.musicController.playSong(
        parser.currentSongId,
        MusicPriorities.PRIORITY_ROOM_PLAYLIST,
        parser.syncCount / 1000,
        0,
        1,
        1,
      );
    ...
    if (parser.nextSongId >= 0)
      Nitro.instance.soundManager.musicController.addSongInfoRequest(
        parser.nextSongId,
      );
  ...
  private onJukeboxSongDisksMessageEvent(
    event: JukeboxSongDisksMessageEvent,
  ): void {
    const parser = event.getParser();

    console.log(
      'Received Jukebox song disks (=playlist) message, length of playlist: ' +
        parser.songDisks.length,
    );
    this._entries = [];
    for (let i = 0; i < parser.songDisks.length; i++) {
      ...
      let songInfo = Nitro.instance.soundManager.musicController.getSongInfo(
        songId,
      ) as SongDataEntry;
      if (songInfo == null) {
        ...
        if (this._missingSongInfo.indexOf(songId) < 0) {
          ...
          Nitro.instance.soundManager.musicController.requestSongInfoWithoutSamples(
            songId,
          );
        }
      }
    ...
    if (this._missingSongInfo.length == 0) {
      Nitro.instance.soundManager.events.dispatchEvent(
        new PlayListStatusEvent(PlayListStatusEvent.PLUE_PLAY_LIST_UPDATED),
      );
    }
  }

  private onJukeboxPlayListFullMessageEvent(
    event: JukeboxPlayListFullMessageEvent,
  ): void {
    console.log('Received jukebox playlist full message.');
    Nitro.instance.soundManager.events.dispatchEvent(
      new PlayListStatusEvent(PlayListStatusEvent.PLUE_PLAY_LIST_FULL),
    );
  }
    ...
    for (let i = 0; i < this.length; i++) {
      ...
      if (songData.id == songInfoEvent.id) {
        ...
        const updatedSongData =
          Nitro.instance.soundManager.musicController.getSongInfo(
            songInfoEvent.id,
          );
  ...
  public requestPlayList(): void {
    Nitro.instance.communication.connection.send(
      new GetJukeboxPlayListMessageComposer(),
    );
  }
