...
export class MusicController implements IMusicController {
  ...
  private static readonly MAXIMUM_NOTIFY_PRIORITY: number =
  ...
  constructor() {
    ...
    this._messageEvents = [
      ...
      new UserSongDisksInventoryMessageEvent(
        this.onSongDiskInventoryMessage.bind(this),
      ),
    ];
  ...
  public init(): void {
    ...
    this._musicPlayer = new MusicPlayer(
      Nitro.instance.getConfiguration<string>('external.samples.url'),
    );

    this._messageEvents.forEach((event) =>
      Nitro.instance.communication.connection.addMessageEvent(event),
    );
    ...
    Nitro.instance.soundManager.events.addEventListener(
      SoundManagerEvent.TRAX_SONG_COMPLETE,
  ...
  public requestUserSongDisks(): void {
    Nitro.instance.communication.connection.send(
      new GetUserSongDisksMessageComposer(),
    );
  }
  ...
  public dispose(): void {
    ...
    this._messageEvents.forEach((event) =>
      Nitro.instance.communication.connection.removeMessageEvent(event),
    );
    ...
    Nitro.instance.soundManager.events.removeEventListener(
      SoundManagerEvent.TRAX_SONG_COMPLETE,
      this.onTraxSongComplete,
    );
  }
  ...
  protected onTraxSongComplete(k: SoundManagerEvent): void {
    ...
    if (this.getSongIdPlayingAtPriority(this._priorityPlaying) == k.id) {
      if (
        this.getTopRequestPriority() == this._priorityPlaying &&
        this.getSongRequestCountAtPriority(this._priorityPlaying) ==
          this._requestNumberPlaying
      ) {
      ...
      if (priorityPlaying >= MusicPriorities.PRIORITY_SONG_PLAY) {
        Nitro.instance.soundManager.events.dispatchEvent(
          new NowPlayingEvent(
            NowPlayingEvent.NPW_USER_STOP_SONG,
            priorityPlaying,
            k.id,
            -1,
          ),
  ...
  private onTraxSongInfoMessageEvent(event: TraxSongInfoMessageEvent): void {
    ...
    for (const song of parser.songs) {
      ...
      if (!songAvailable) {
        ...
        const songInfoEntry: SongDataEntry = new SongDataEntry(
          song.id,
          song.length,
          song.name,
          song.creator,
        ...
        const songId: number = this.getSongIdRequestedAtPriority(
          topRequestPriotityIndex,
        );
        if (song.id === songId) {
          this._musicPlayer
            .preloadSamplesForSong(song.data)
            .then(() => this.onSongLoaded(song.id));
          //this.playSongObject(topRequestPriotityIndex, songId);
        }
        Nitro.instance.soundManager.events.dispatchEvent(
          new SongInfoReceivedEvent(
            SongInfoReceivedEvent.SIR_TRAX_SONG_INFO_RECEIVED,
            song.id,
          ),
        );
        while (this._diskInventoryMissingData.indexOf(song.id) != -1) {
          this._diskInventoryMissingData.splice(
            this._diskInventoryMissingData.indexOf(song.id),
            1,
          );
          if (this._diskInventoryMissingData.length === 0) {
            Nitro.instance.soundManager.events.dispatchEvent(
              new SongDiskInventoryReceivedEvent(
  ...
  private onSongDiskInventoryMessage(
    event: UserSongDisksInventoryMessageEvent,
  ): void {
    const parser = event.getParser();
  ...
  private onTick(): void {
    ...
    Nitro.instance.communication.connection.send(
      new GetSongInfoMessageComposer(...this._songRequestList),
    );
  ...
  public playSong(
    ...
    if (priority >= this._priorityPlaying) {
      ...
      console.log(
        'Higher priority song blocked playing. Stored song ' +
          songId +
          ' for priority ' +
          priority,
      );
  ...
  private playSongObject(priority: number, songId: number): boolean {
    if (
      songId == -1 ||
      priority < 0 ||
      priority >= MusicPriorities.PRIORITY_COUNT
    ) {
      return false;
    }
    let _local_3 = false;
    ...
    if (!songData) {
      console.log(
        'WARNING: Unable to find song entry id ' +
          songId +
    ...
    if (_local_3) {
      console.log(
        'Waiting previous song to stop before playing song ' + songId,
      );
      //return true;
    }
    ...
    const songRequestData: SongStartRequestData =
      this.getSongStartRequest(priority);
    ...
    if (priority > MusicPriorities.PRIORITY_ROOM_PLAYLIST) {
      Nitro.instance.soundManager.events.dispatchEvent(
        new NowPlayingEvent(
          NowPlayingEvent.NPE_USER_PLAY_SONG,
          priority,
          songData.id,
          -1,
        ),
  ...
  private notifySongPlaying(k: SongDataEntry): void {
    ...
    if (
      ...
      (!(this._previousNotifiedSongId == k.id) ||
        timeNow > this._previousNotificationTime + _local_2)
    ) {
      Nitro.instance.soundManager.events.dispatchEvent(
        new NotifyPlayedSongEvent(k.name, k.creator),
      );
      this._previousNotifiedSongId = k.id;
  ...
  private addSongStartRequest(
    ...
    const songStartRequest = new SongStartRequestData(
      songId,
      startPos,
      playLength,
      fadeInSeconds,
    ...
    this._songRequestCountsPerPriority[priority] =
      this._songRequestCountsPerPriority[priority] + 1;
    return true;
  }
  ...
  private getSongIdRequestedAtPriority(priorityIndex: number): number {
    if (priorityIndex < 0 || priorityIndex >= MusicPriorities.PRIORITY_COUNT)
      return -1;
  ...
  private reRequestSongAtPriority(k: number): void {
    this._songRequestCountsPerPriority[k] =
      this._songRequestCountsPerPriority[k] + 1;
  }
  ...
  private onJukeboxInit(k: Event): void {
    this.disposeRoomPlaylist();
    this._roomItemPlaylist =
      new JukeboxPlaylistController() as IPlaylistController;
    this._roomItemPlaylist.init();
    Nitro.instance.communication.connection.send(
      new GetNowPlayingMessageComposer(),
    );
  }
