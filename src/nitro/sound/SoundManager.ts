...
export class SoundManager extends NitroManager implements ISoundManager {
  ...
  private onEvent(event: INitroEvent) {
    switch (event.type) {
      case RoomEngineSamplePlaybackEvent.PLAY_SAMPLE: {
        ...
        this.playFurniSample(
          castedEvent.objectId,
          castedEvent.sampleId,
          castedEvent.pitch,
        );
        return;
      }
      ...
      case NitroSettingsEvent.SETTINGS_UPDATED: {
        ...
        const volumeFurniUpdated =
          castedEvent.volumeFurni !== this._volumeFurni;
        ...
        if (volumeFurniUpdated)
          this.updateFurniSamplesVolume(this._volumeFurni);

        if (volumeTraxUpdated)
          this._musicController?.updateVolume(this._volumeTrax);
  ...
  private playFurniSample(objectId: number, code: number, pitch: number): void {
    ...
    if (!sample) {
      const sampleUrl = NitroConfiguration.getValue<string>(
    ...
    if (!this._furnitureBeingPlayed.hasKey(objectId))
      this._furnitureBeingPlayed.add(objectId, code);
