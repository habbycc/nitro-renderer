...
export class RoomCameraWidgetManager implements IRoomCameraWidgetManager {
  ...
  public init(): void {
    ...
    const imagesUrl =
      NitroConfiguration.getValue<string>('image.library.url') +
      'Habbo-Stories/';
    const effects = NitroConfiguration.getValue<
      {
        name: string;
    ...
    for (const effect of effects) {
      ...
      const cameraEffect = new RoomCameraWidgetEffect(
        effect.name,
        effect.minLevel,
      );
    ...
    this.events.dispatchEvent(
      new RoomCameraWidgetManagerEvent(
        RoomCameraWidgetManagerEvent.INITIALIZED,
      ),
    );
  }
