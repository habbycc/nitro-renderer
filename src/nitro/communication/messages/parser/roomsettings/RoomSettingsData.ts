...
export class RoomSettingsData {
  ...
  public get controllerList(): FlatControllerData[] {
    if (!this._controllerList) {
      ...
      for (const controllerData of this._controllersById.values())
        this._controllerList.push(controllerData);
  ...
  public get bannedUsersList(): BannedUserData[] {
    if (!this._bannedUsersList) {
      ...
      for (const bannedUserData of this._bannedUsersById.values())
        this._bannedUsersList.push(bannedUserData);

      this._bannedUsersList.sort((a, b) =>
