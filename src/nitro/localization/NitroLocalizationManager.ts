...
export class NitroLocalizationManager
    extends NitroManager
    implements INitroLocalizationManager
{
    ...
    protected onInit(): void {
        this._communication.registerMessageEvent(
            new BadgePointLimitsEvent(this.onBadgePointLimitsEvent.bind(this)),
        );

        let urls: string[] =
            NitroConfiguration.getValue<string[]>('external.texts.url');
        ...
        for (let i = 0; i < urls.length; i++)
            urls[i] = NitroConfiguration.interpolate(urls[i]);
        ...
        if (!this._pendingUrls.length) {
            this.events &&
                this.events.dispatchEvent(
                    new NitroLocalizationEvent(NitroLocalizationEvent.LOADED),
                );
    ...
    private onLocalizationLoaded(
        ...
        if (!this.parseLocalization(data))
            throw new Error(`Invalid json data for file ${url}`);
    ...
    private onLocalizationFailed(error: Error): void {
        this.events &&
            this.events.dispatchEvent(
                new NitroLocalizationEvent(NitroLocalizationEvent.FAILED),
            );
    }
    ...
    private onBadgePointLimitsEvent(event: BadgePointLimitsEvent): void {
        ...
        for (const data of parser.data)
            this.setBadgePointLimit(data.badgeId, data.limit);
    }
    ...
    public getValue(key: string, doParams: boolean = true): string {
        ...
        if (keys && keys.length) {
            for (const splitKey of keys)
                key = key.replace(
                    splitKey,
                    this.getValue(splitKey.slice(2, -1), doParams),
                );
        }
        ...
        if (parameters) {
            for (let i = 0; i < parameters.length; i++) {
                ...
                if (value.startsWith('%{')) {
                    ...
                    if (
                        indexKey === -1 ||
                        typeof result[indexKey] === 'undefined'
                    )
                        continue;
    ...
    public registerParameter(
        ...
        if (!key || key.length === 0 || !parameter || parameter.length === 0)
            return;
